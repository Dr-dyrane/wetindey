...
#### Docs-truth conformance wave 1 - ACTIVE

Owner: Private Contractor, Maps Delivery `c9c17443-ef5e-4a7b-9b6e-c8f5381da30c`, from the docs-truth sweep; the ADR-023 header-drift class, now found in four more ADRs plus onboarding and product docs. Record-only corrections, decision text untouched everywhere: ADR-020 header records the shipped nonce CSP implementation; ADR-021 header records P1/P2 shipped inert with P3 unclaimed; ADR-022 header records the app-layer half at bcf6028 with the schema half gated; ADR-028 status records the implemented default-off lane; SEO.md drops the three words claiming the deleted zzboom block; USER-FLOW.md removes the AUDIT DEFECT annotation for the fixed-and-verified ADR-023 defect and corrects its header sentence; README.md fixes the env-var bootstrap claim (the cookie secret is build-required), rewrites the scripts table, and deletes the false no-tests paragraph. Exact paths: `docs/adr/020-per-request-nonce-content-security-policy.md`; `docs/adr/021-account-deletion-lifecycle.md`; `docs/adr/022-earned-seller-and-role-onboarding.md`; `docs/adr/028-contribution-evidence-media.md`; `docs/product/SEO.md`; `docs/product/USER-FLOW.md`; `README.md`. Routed, not claimed: the SERVICE-ARCHITECTURE amendment block (wave 2), source-comment file+symbol citation conversion (wave 3), the maps-location forward entry (own tuple ceremony), and the human-interface dead-command action (HI seat's).
