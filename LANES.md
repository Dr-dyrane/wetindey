...
| **H6 CSP architecture assessment** | complete read-only/pathless; implementation unclaimed pending ADR-020 acceptance and exact ownership | Security & Privacy Engineering Lead `019f75e5-8c04-74c0-9392-d9f741a7a131` | P0: avoid intersecting static Vercel plus middleware CSPs; nonce all three raw layout scripts and parser-inserted Mapbox script; clone the same Next 15.5.20 policy onto request headers and response because `x-nonce` alone is insufficient | Future handoff begins with `docs/adr/020-per-request-nonce-content-security-policy.md`, `DECISIONS.md`, `WETINDEY_BIBLE.md`, `docs/architecture/SERVICE-ARCHITECTURE.md`, then exact implementation ownership for `src/middleware.ts`, `src/app/layout.tsx`, `vercel.json`, and conditional SEO/error/route/service-worker/report-endpoint call sites. Dynamic/no-store HTML, environment policies, Blob CSP, and report-only rollout remain explicit dependencies. No paths claimed |
...
| **Nearby Presence first slice / location-source and privacy policy** | exact seven-path proposal reconciled but wholly unclaimed because `page.tsx` conflicts with Contribution | Presence Platform Engineer `019f759f-3521-7ee1-90a3-5af3539d757e` waiting | Browsing fallback/selection never authorizes presence. A selected Badagry center may show local information or explicitly Sample users only. Real peers require each viewer’s fresh foreground GPS in the same area, reciprocal opt-in, and coarse lease-scoped markers; interaction/contact never automatically reveals true location | Peer presence remains fail-closed through `sharedUsers={[]}`. Optional country/region text remains explicit and counsel-gated. Exact path statuses and disposable proof gates are recorded below. No edits, `0012`, shared migration, pilot traffic, push, or deploy |
...
| **H6 CSP architecture assessment** | Security & Privacy Engineering Lead `019f75e5-8c04-74c0-9392-d9f741a7a131`; Plato assessment `019f763f-ad8d-7fc3-a7b2-54ba73c63acd` | ✅ read-only assessment complete; implementation unclaimed pending ADR acceptance and exact ownership | — | Security/privacy architecture | 2026-07-18 | P0: static `vercel.json` CSP plus future middleware would create two intersecting enforcing policies; all three raw layout scripts and parser-inserted Mapbox script require nonce; Next 15.5.20 requires the same policy on cloned request headers and response, so `x-nonce` alone is insufficient. Future governance paths are `docs/adr/020-per-request-nonce-content-security-policy.md`, `DECISIONS.md`, `WETINDEY_BIBLE.md`, and `docs/architecture/SERVICE-ARCHITECTURE.md`; later exact implementation ownership must cover `src/middleware.ts`, `src/app/layout.tsx`, `vercel.json`, and conditional SEO/error/route/service-worker/report-endpoint call sites. Dynamic/no-store HTML, environment-specific policies, Blob CSP, and report-only rollout remain P1/P2 dependencies. No paths claimed; no edits/tests/browser/DB. |
...
| **Nearby Presence first-slice proposal / privacy policy** | Presence Platform Engineer `019f759f-3521-7ee1-90a3-5af3539d757e` | 🟡 exact seven-path proposal reconciled; implementation unclaimed due `page.tsx` conflict | — | ADR-016 private-pilot preparation | 2026-07-18 | Manual/AI/default centers, including selected Badagry, never authorize real peer presence. Badagry browsing may show local information or explicitly Sample users only. Real presence requires each viewer’s fresh foreground GPS in the same area, reciprocal opt-in, coarse lease-scoped markers, and no automatic true-location disclosure on interaction/contact. Optional country/region text remains explicit and counsel-gated. `sharedUsers={[]}` remains fail-closed. No `0012`, shared migration, pilot traffic, push, or deploy. |
