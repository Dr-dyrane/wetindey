...
| **phase-0 / orphans** | *unclaimed* | ⚪ blocked | `src/modules/**`, `src/core/module-contract.ts`, `src/core/*/`, `src/integrations/*/`, `knip` config, CI | Phase 0 | — | **Blocked by the auth conflict** — Phase 0 removes `jotai` from `page.tsx`, which auth owns. Do not start until auth lands or releases. |
