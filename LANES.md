...
#### Docs-truth conformance wave 3: source-comment citation rot - ACTIVE

Owner: Private Contractor, Maps Delivery `c9c17443-ef5e-4a7b-9b6e-c8f5381da30c`. Comment-only conversion of dead citations to the file-and-symbol convention .env.example itself mandates ("Readers below are cited by FILE and SYMBOL, never by line number"). Known dead cites from the sweep: trust.ts references getFoodItemCandidates and actions.ts lines that no longer exist (including a warning about deleted arithmetic); validation.ts carries about ten actions.ts line cites for functions now living in _actions files; strings.ts cites the deleted AreaPickerSheet.tsx; perf.ts, MapboxCanvas.tsx, Skeleton.tsx, and error.tsx cite page.tsx line numbers from the god-file era. Zero behavior change: comments only, every replacement verified to name a real current file and symbol, and any comment warning about since-deleted code is either deleted or rewritten as history with its resolving commit. Exact paths: `src/lib/trust.ts`; `src/lib/validation.ts`; `src/core/i18n/strings.ts`; `src/lib/perf.ts`; `src/design-system/components/MapboxCanvas.tsx`; `src/design-system/components/Skeleton.tsx`; `src/app/error.tsx`. Completion: word-diff proves comment-only, every new cite resolves, gates green, independent default-to-REFUTED refutation, path-scoped commit, release.
