...
| **governance / ADR-006 citations (H23)** | this session | 🟢 active | `docs/adr/006-freshness-windows.md`, `LANES.md` | H23 | 2026-07-17 | **Taking H23.** Verified: **17 line citations in ADR-006**, and the named ones have drifted — `rankByConfidence` 514-527 → **591**, `getOfferTrustBatch` 1295 → **1607**, `ageDecay` 225-230 → **231**. Every symbol still resolves by name. The stakes are the point: ADR-006 is the document that stopped a session deleting `rankByConfidence` as an orphan, and an ADR whose citations do not resolve cannot do that again. Fix is the one I already proved on `README`/`.env.example` — **cite the symbol, not the line**. Also fixing a bug in this file: the *Claiming a lane* example row renders as a real lane called **my-lane**, owned by "session name or your name". |
| **type / weight discipline** | this session | 🟡 partial — see H26 | — | owner directive 2026-07-17 | 2026-07-17 | **Owner: too many weights; muted must never be semibold; status normal; stop repeating bold/demibold — it is not a calm UI.** Counted: **28 `font-semibold`**, 15 `font-medium`, 2 `font-normal`, 1 `bold`, 1 `black`. A weight used 28 times is not hierarchy, it is texture. Only `headline` bakes weight into the type scale (600); every other one is a class layered on top. Doing: status badge → normal, and muted labels drop `font-medium`. **Not touching `page.tsx` (font-bold + font-black) or `ProfileSheet.tsx`** — auth/map lanes own both; handed off. |
| **a11y / input focus (P1-1)** | this session | ✅ done (SearchField browser-unverified — see H24) | — | ACCESSIBILITY P1-1 | 2026-07-17 | **The search field and the price input delete their focus ring and replace it with a 1.15:1 cue.** Same class as P0-1 but worse: `focus:outline-none`, not `focus-visible:` — it kills the ring for keyboard users too, with no carve-out. The replacement is the fill lifting `fillTertiary`→`surface`, which composites `(239,239,240)`→`#FFFFFF` = **1.15:1** (verified; SC 1.4.11 wants 3:1). `SearchField`'s own docstring says the intent is "how iOS does it" — **the intent is right and the values do not deliver it**, and iOS pairs that lift with a caret and a rising keyboard, neither of which is a focus indicator on the web. These are the app's front door and its only write path. Both files unowned. **Not taking P0-7 or P0-8** — `text-onStatus` is used only in `MapboxAdapter.ts:476-480` and `page.tsx`, both the map lane's. | ACCESSIBILITY P0-6 | 2026-07-17 | **`globals.css:101` says the `-fg` tokens are "Darkened for small text on a tint". They are not, for three of four.** Recomputed independently, matching the audit: `confirmed` **3.92 / 3.55**, `caution` 4.64 / **4.21**, `unavailable` 4.88 / **4.39**, `info` 5.43 / 4.92 — against the 4.5:1 small-text floor on `surface` / `background`. `confirmed` is the badge that says a price is trustworthy and it is the least readable of the four. **Fixing the COMMENT, not the colours** — changing the palette is the same fidelity-vs-compliance call the audit says is the owner's (P0-5), and I am not taking it. A comment that asserts a false measurement is a bug on this repo's own terms. | H18 / P0-1 | 2026-07-17 | **Resolved H18 by bisecting the class list: `transition-all` is what eats the focus ring.** It animates `outline`, and the outline never lands on the target — it sits at `currentColor solid 1.5px, offset 0` instead of `#007AFF solid 2px, offset 2px`. Stable at 0/100/400/600ms, so it is not a slow transition; it just never arrives. A bare `duration-*` does it too — `transition-duration` with no `transition-property` defaults to `all`. Tailwind's plain `transition` covers colour, opacity AND transform (everything Button animates) and excludes `outline`. Finishing the P0-1 fix I left half-done. | ACCESSIBILITY P0-1 | 2026-07-17 | **`Button` turns off the one stroke this design system permits.** `focus-visible:outline-none` on the base class beats the global `:focus-visible` ring on source order AND specificity, and in a zero-border UI there is no border or shadow to stand in for it — the focused state is pixel-identical to the unfocused one. **Verified live on "Submit Report", the app's only write path: `focused:true`, `outlineStyle:"none"`, `boxShadow:"none"`.** AGENTS.md's DoD says "focus outlines are complete"; they are absent. `Button.tsx` unowned. **Not touching `ModalSheet.tsx` (P0-2) or `MapboxAdapter.ts` (P0-4)** — map/sheet lane owns both. |
| **a11y / pinch-zoom (P0-3)** | this session | ✅ done | — | ACCESSIBILITY P0-3 | 2026-07-17 | **Removing `userScalable:false` + `maximumScale:1`.** Documented as a P0 WCAG 2.1 AA failure (SC 1.4.4) by our own audit and never fixed. The audit declined it as *"a product decision — someone chose this"*. **The history says nobody did:** it arrives in the INITIAL COMMIT as boilerplate, and it is the only non-obvious line in that file with no comment justifying it, in a file where the theme-color block gets six. Android Chrome honours it; iOS ignores it — so it is Android-only, which is most of the pilot's audience. `layout.tsx` unowned. |
| **h15 / secure-context boot crash** | this session | ✅ done | — | H15 | 2026-07-17 | **Taking H15 — it IS agent-verifiable, and the belief that it was not is what kept it open.** My own refuter reproduced it in ~90s: `mcp__claude-in-chrome__` at **`http://192.168.1.71:3000`** (NOT localhost) gives `isSecureContext:false`, `randomUUID:undefined`, the *Something scatter* boundary, on a page titled WetinDey. `layout.tsx` is unowned (0 mentions here) and already carries two blocking inline scripts, so the slot and the precedent exist. **Not touching `package.json`** — `--experimental-https` is the auth lane's file. |
| **governance / contact-model ADR** | this session | ✅ done (rejected) | — | H14 | 2026-07-16 | **Writing the ADR H14 asked for.** Re-verified every claim against the live DB before starting, because the handoff's own headline was that its first argument was false: 38/60 `open_market`, contacts NULL in **60/60** with 0 non-private, **13/13 kiosks carry an address**, RLS false, `pg_policies` = 0. Docs only; touching no source. |
...
| H18 | ~~design system focus ring~~ | **CLOSED 2026-07-17 — and my own diagnosis was wrong twice before it closed.** The culprit was **`transition-all`**, found by bisecting `Button`'s class list one class at a time. It transitions `outline-color`/`-width`/`-offset`, so the ring FADES IN over 160ms instead of appearing; for those 160ms it sits at its from-value — a 1.5px `currentColor` hairline at offset 0, which on `bg-accent` is white on blue and invisible. **It does arrive.** My H18 text said it never lands; a refuter disproved that by forcing the transition to `.finish()` and watching it hit `rgb(0,122,255) solid 2px, offset 2px`. My "stable at 0/100/400/600ms" reading was the first frame, four times — very likely a throttled background tab. Fixed with plain `transition`, whose explicit property list excludes `outline` and includes `transform`, so the press still animates. **Verified with real keys on `Submit Report` AND `ReportPriceSheet`'s segmented control: `rgb(0,122,255) solid 2px, offset 2px`.** |
