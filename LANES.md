...
| Human Interface Design Engineer | Active, exact two-path retained Market Details lane | `8b0d5e4` passed static refutation but runtime refuted level-1 scroll reset and containment; retain only `page.tsx` and `PlaceOfferRow.tsx` while the disjoint Motion lane corrects NavigationStack |
...
| Iconography & Visual Systems Lead | Active, exact five-path intent-adoption lane | Item Detail/Get-It completed at `ed5a6cb` with static and combined runtime PASS; those two paths are released. Continue only across Settings, SheetPicker, CurrencyPicker, ExchangePanel, and MapLoader |
...
| Motion & Interaction Engineer | Active, exact two-path Market Details scroll-containment sub-lane | Own only `NavigationStack.tsx` and `scripts/motion-contracts.test.ts`; do not claim `BottomSheet.tsx` |
...
- **CSP collector operations — PASS:** Production and Preview hold sensitive
  `CRON_SECRET` and `BLOB_READ_WRITE_TOKEN_PRIVATE`. The collector uses a separate private
  Blob store and never falls back to the project-connected public avatar store. Hourly
  cleanup is enabled at `0 * * * *`. The active exact WAF rule is
  `POST /reports/security/csp`, 60 requests per IP per 60 seconds, with platform `429`
  semantics and no pending draft; the platform maintainer owns rollback. Production
  remains fail-open and report-only. Vercel counters being per-region remains the
...
- **Shared database migration:** the deterministic exact-target flow and migration window
  ...
  refutation pass. For corrected-unapplied Presence `0012`, source/disposable validation
  has passed at `d5991c9`; its remaining shared-rollout blocker is solely exact target
  identity, execution role, and scheduler proof. No shared action may occur before that
  proof is recorded.
...
| Contribution containment/integrity (`495750a`) | **Closed / released with controller runtime PASS** | The runtime artifact closes containment. `src/app/actions.ts`, `src/app/page.tsx`, `ReportPriceSheet.tsx`, `ConfirmVisitSheet.tsx`, and `src/core/i18n/strings.ts` are released; only `page.tsx` later transferred to the retained Market Details claim below |
...
| Nearby Presence forward `0012` | **Closed / exact four source paths released; shared rollout pathless** | Corrected unapplied migration `d5991c9` is on `origin/main`; source and disposable validation are complete. Both presence pillars, `0012_guarded_presence.sql`, and the focused migration contract are released. Shared rollout remains blocked solely on exact target identity, role, and scheduler proof; no database action is authorized |
...
| Market Details — Place Offer Surface Redesign | **Active / retained exact two-path slice; static PASS, runtime REFUTED** | Commit `8b0d5e4` passed static refutation, but compact runtime evidence shows the persistent level-1 scroller opens at a stale position and terminal scroll escapes to the document. Persistent Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d` retains only `src/app/page.tsx` and `src/design-system/components/PlaceOfferRow.tsx`; `src/app/actions.ts` is released. Self-location/avatar and H38 remain pathless behind `page.tsx`; H31 is queued and unclaimed on the released actions path |
| Market Details level-1 scroll reset and containment | **Active, exact two-path disjoint Motion sub-lane** | Motion & Interaction Engineer owns only `src/design-system/components/NavigationStack.tsx` and `scripts/motion-contracts.test.ts`. Reset the persistent level-1 scroller on each closed→open push or new detail without resetting level 0; contain terminal wheel, touch, and programmatic scrolling from the document. Preserve push/pop animation, held-detail lifetime, focus/inert semantics, regular layout, BottomSheet API, and one content instance. `BottomSheet.tsx` is explicitly unclaimed |
...
Completed commit `d5991c9` changed exactly:
...
The correction closed the five executable SQL failures previously found by disposable
execution:
...
Source and disposable validation are complete, including deterministic idle-retention
cleanup, account-deletion purge/anonymization, fresh blank and upgrade Drizzle lineages,
all nine RPCs, lifecycle, security, cleanup, idempotence, and independent new-cluster
refutation. All four source/contract paths are released.

The shared migration remains unapplied. Its only remaining rollout blocker is pathless
proof of the exact target identity, execution role, and scheduler capability/configuration.
No shared database action, source edit, push, deployment, or pilot authority follows from
the completed disposable evidence. `src/db/schema/presence.ts`,
`src/db/schema/index.ts`, `src/db/migrations/meta/0012_snapshot.json`,
`src/db/migrations/meta/_journal.json`, and every `0011` byte remain outside this completed
scope and unclaimed.

### Closed lane: Search/provenance disposable SQL evidence

The exact deployed-current-main candidate `97b74af` passed disposable SQL execution,
including the strengthened unavailable and contradictory cases and exact cleanup. The
pathless execution gate is closed and released. This is evidence for the Search/provenance
candidate only; it creates no code, schema, shared-target, push, deployment, or continuing
database claim. The Search & Ranking employee remains persistent and available.

### Closed source lane: production nonce CSP P1 correction

The now-released experiment used exactly:

- `src/middleware.ts`
- `src/lib/seo.tsx`
- `scripts/csp-policy-contracts.test.ts`

Ready Preview `dpl_DXZqqwy3srE6E6Ju8XLJ968otyf7` proved `_middleware` runs as a
`nodejs24.x` Lambda. Two authenticated fresh HTML requests each returned 29 scripts: six
app-authored scripts carried nonces and 23 Next bootstrap, Flight, and chunk scripts were
bare. Each response retained exactly one unchanged enforcing CSP and one report-only CSP,
with no internal headers exposed.

The experimental line was reverted; `src/middleware.ts`, `src/lib/seo.tsx`, and
`scripts/csp-policy-contracts.test.ts` are clean and released. The remaining P1 is
**REFUTED/pathless at the upstream Next/Vercel adapter boundary**, not an active source
lane. Keep the collector report-only and prohibit enforcement. Reopen source work only
when a Next/Vercel adapter or version change supplies materially new evidence; do not
repeat the identical experiment.

### Closed lane: Identity & Email Delivery — branded Neon Auth OTP

The completed implementation used exactly:

- `src/app/api/webhooks/neon-auth/route.ts`
- `src/lib/auth-email.ts`
- `src/lib/auth-email-logo.ts` added by the completed embedded-logo follow-up
...
`public/logo.png` remains unclaimed. The released email does not load it or any remote
image at runtime; the monochrome mark is embedded in `src/lib/auth-email-logo.ts`.
...
Implementation commit `64d3519` and type correction `8583495` are pushed to
`origin/main`. Independent default-to-refuted review returned **VERIFIED** with no
P1/P2/P3. `npm run test:auth-email`, exact-path ESLint, and `npm run build` passed.
Hostinger SMTP authenticated with backing mailbox `support@wetindey.live` and visible
alias `WetinDey <auth@wetindey.live>`; the required Vercel variables are configured in
Production and Preview.

Production deployment `dpl_APdTiwQR35NPcTxXVdTFPY9KaFrd` reached **Ready**. The live
route returns `415` for invalid media and `405` for `GET`. Neon main branch
`br-flat-band-aui9waf5` has Webhooks enabled for application `WetinDey`, URL
`https://www.wetindey.live/api/webhooks/neon-auth`, event `Send OTP`, and a ten-second
timeout. A real production OTP was delivered to Gmail and successfully completed
sign-in. This closed and released the original six implementation paths while preserving the
existing auth proxy/client and `ProfileSheet` flow, Neon OTP generation and verification
semantics, `.env` files, database/schema/migrations, Account Deletion, Security CSP,
Motion, and Maps outside the completed scope.

The Founder rejected the original green visual treatment. Forward correction `d549d42`
(`fix(auth): align OTP email with monochrome theme`) is pushed on `origin/main` and changed
only `src/lib/auth-email.ts` plus `scripts/auth-email-contracts.test.ts`. The released
design is true black/white neutral with a text wordmark, fluid mobile-safe shell, stronger
small-text contrast, and no remote imagery or colour leakage. `npm run test:auth-email`
passed, and a fresh independent default-to-refuted reviewer returned **VERIFIED** with no
P1/P2/P3.

Production deployment `dpl_D7NXRhdh4eiUniiVEiEtdxUsyCnH` is **Ready** and aliased to
`www.wetindey.live`. A fresh production OTP `POST` returned `200`; Gmail confirmed a new
unread message from `WetinDey auth@wetindey.live` with exact subject
`Your WetinDey verification code`. This forward correction adds production evidence and
does not reopen either source path or any other released path.

After approving the monochrome treatment, the Founder requested the WetinDey logo.
Follow-up `712719f` (`fix(auth): embed monochrome OTP logo`) is pushed on `origin/main`
and changed exactly `src/lib/auth-email.ts`, new `src/lib/auth-email-logo.ts`, and
`scripts/auth-email-contracts.test.ts`. It embeds the real monochrome mark as a fixed
96×96 PNG through standards-shaped CID `wetindey-logo@auth.wetindey.live`; no remote image
URL is used.

The focused contract independently verifies the PNG signature, chunk CRCs,
IHDR/IDAT ordering and IEND, decompression, scanline filters, pixel-level grayscale, a
fixed test-local SHA, CID-only HTML, and inline-attachment metadata. The focused contract
passed and the independent final reviewer returned **VERIFIED** with no P1/P2/P3.
Production deployment `dpl_B7QwNqAYTToHsAXsVCM5jh1JXwRM` is **Ready** and aliased to
`www.wetindey.live`; a fresh OTP returned `200`. Gmail confirmed a new unread message
with inline image `wetindey-logo.png`, 3334 bytes, exact CID
`wetindey-logo@auth.wetindey.live`, and `has_attachment=false`. All three follow-up paths
are released.

Residual operational follow-up remains: caches are process-local; deterministic
Message-ID does not provide exactly-once delivery across serverless instances; the
provider capability is Beta; and SPF/DKIM/DMARC monitoring plus ongoing email-client
rendering checks remain required. These residuals do not reopen an implementation path.

### Closed lane: H37 semantic place-marker symbols

Completed commit `6611068` changed exactly:

- `src/design-system/components/MapboxCanvas.tsx`
- `src/integrations/maps/MapboxAdapter.ts`

The implementation carries `placeType` into deterministic semantic symbols, preserves
accessible names and the generic unknown-type fallback, and received an independent
**NOT REFUTED** verdict. Both exact paths are released.

Runtime scope remains honest: `src/app/page.tsx` supplies `sharedUsers={[]}` and
`getSharedUserLocations()=[]`, so peer avatar markers are not live. Self-avatar behavior
is a separate unclaimed lane. The later contextual-orb/selected-glow concept is not
...
### Active lane: Market Details — Place Offer Surface Redesign

Persistent Human Interface Design Engineer
`019f75a5-0fc6-7f40-9a0f-8097ead3b45d` owns exactly:

- `src/app/page.tsx`
- new `src/design-system/components/PlaceOfferRow.tsx`

This is one short atomic slice for the actual place/market detail surface rejected by the
Founder. Commit `8b0d5e4` passed static refutation, but compact runtime refutation found
that the persistent level-1 scroller opens stale and terminal scroll escapes to the
document, so these two paths remain retained. Remove the artificial `max-h-[40vh]` dead
zone; preserve a pinned `Get it`; add
licensed item imagery or a deterministic Food fallback; and carry truthful price range,
unit, availability, freshness, last-observed, and provenance semantics through loading,
empty, and error states. Acceptance includes compact and desktop, light and dark,
accessible 44px targets, imagery fallback, independent visual/accessibility and
source/data refutation, and the ADR-001 fulfilment exclusion.

Do not widen into `src/app/actions.ts`, `ItemCard`, `ListRow`, `IconOrb`,
self-location/avatar, H38, H31,
Presence, schema, migrations, fulfilment, or another category abstraction. Self-location/
avatar and H38 remain pathless and wait for this slice to release `page.tsx`; H31 remains
queued and unclaimed on the released actions path.

### Active sub-lane: Market Details level-1 scroll reset and containment
...
Reset the persistent level-1 scroller to the top on every closed→open push and whenever a
new detail is pushed, without resetting the level-0 list. Prevent terminal wheel, touch,
and programmatic scroll from chaining to the document. Preserve the existing push/pop
animation, held-detail lifetime, focus and inert semantics, regular layout, BottomSheet
API, and a single content instance. `src/design-system/components/BottomSheet.tsx` is
explicitly unclaimed. Acceptance requires a focused contract and fresh independent
refutation before either path is released.

### Queued lane: self-location avatar marker — signed-in and guest
...
and authorizes no edit while Market Details owns `src/app/page.tsx`. The H37 Maps paths
are released, but they do not transfer into this queued lane without a fresh exact handoff.

Implementation begins only after Market Details releases `src/app/page.tsx` and the
controller records a fresh exact claim. The intended vertical slice remains:
...
The completed foundation used exactly:
...
Direct existing-tab Category screenshots passed light, dark, and grayscale runtime
refutation: Food maps warm, Money maps teal, disabled/future categories remain neutral,
solid glyphs remain legible, and there is no rainbow spill. The focused contract passed
the 44px token, forced-colors/reduced-motion, and glow semantics. All five paths are
released. Runtime forced-colors emulation remains an evidence residual only and does not
retain a path.

### Closed lane: Iconography solid-glyph adoption — Item Detail/Get-It

Commit `ed5a6cb` changed exactly:
...
### Active lane: Iconography intent adoption — remaining sheets and MapLoader

The persistent Iconography & Visual Systems Lead
`019f75cf-4d8f-7263-822b-06e846fce7ac` owns exactly:
...
Apply contextual dimensional solid orbs per ADR-018 while keeping universal utilities
neutral and authentic currency flags unchanged. Preserve copy, data, actions,
interactions, layout, and motion. Do not touch page, actions, maps, database, schema, or
migrations. Acceptance requires light, dark, grayscale, forced-colors, reduced-motion,
44px, and accessibility refutation followed by one path-scoped commit.

### Closed lane: Modal zero-focusable focus containment

Completed commit `f19990b` changed exactly:

- `src/design-system/components/ModalSheet.tsx`
- `scripts/motion-contracts.test.ts`

The correction contains initial focus, Tab, and Shift+Tab inside the dialog even when all
body controls are disabled, while preserving Escape, Close, trigger restoration,
nested/pushed sheets, motion, stacking, and visual layout. The Safari refuter returned
**PASS**. `ModalSheet.tsx` remains released; `scripts/motion-contracts.test.ts` later
transferred to the active disjoint Market Details scroll-containment lane. No caller,
page, action, database, map, or iconography ownership was created.
...
| **Iconography intent adoption — remaining sheets and MapLoader** | 🟢 active exact five-path lane | persistent Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac`; visual/AX refuter | `src/app/_components/SettingsSheet.tsx`; `src/design-system/components/SheetPicker.tsx`; `src/app/_components/CurrencyPickerSheet.tsx`; `src/app/_components/ExchangePanel.tsx`; `src/design-system/components/MapLoader.tsx` | Contextual dimensional solid orbs per ADR-018; universal utilities neutral; authentic currency flags unchanged; no copy/data/actions/interaction changes; require light/dark/grayscale/forced-colors/reduced-motion/44px/AX refutation |
...
| **Self-location avatar marker — signed-in and guest** | 🟡 named queued vertical; responsibility assigned, paths unclaimed until Market Details releases `page.tsx` and a fresh handoff lands | persistent Maps Engineering Lead implementation owner; Iconography & Visual Systems Lead visual reviewer; independent Privacy refuter | Intended post-release paths: `src/app/page.tsx`; `src/design-system/components/MapboxCanvas.tsx`; `src/integrations/maps/MapboxAdapter.ts`; conditional new `src/design-system/components/SelfLocationMarker.tsx` only if implementation proves it useful; new `scripts/map-self-location-contract.test.ts` | Signed-in self uses uploaded avatar or local initials fallback; guest self uses a bundled/local “me” fallback. Precise GPS gets bounded precise treatment; approximate/area uses an identity orb plus diffuse accuracy halo, never blur instead of identity. Require truthful screen-reader labels, light/dark/grayscale, non-obstruction, no external avatar generator, peer publication, Presence rollout, false precision, DB/schema/migration. Released Maps paths remain unclaimed until the fresh post-Market handoff |
| **Market Details — Place Offer Surface Redesign** | 🟠 active/retained exact two-path slice; `8b0d5e4` static PASS, runtime REFUTED | persistent Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; independent visual/accessibility and source/data refuters | `src/app/page.tsx`; new `src/design-system/components/PlaceOfferRow.tsx` | Truth, imagery/fallback, pinned Get it, and compact layout remain retained. Runtime refutation: the persistent level-1 scroller opens stale and terminal scroll escapes to the document. `actions.ts` is released; self-location/avatar and H38 wait pathless behind `page.tsx`; H31 remains queued/unclaimed |
| **Market Details level-1 scroll reset and containment** | 🟢 active exact two-path disjoint Motion sub-lane | Motion & Interaction Engineer; fresh independent refuter | `src/design-system/components/NavigationStack.tsx`; `scripts/motion-contracts.test.ts` | Reset level 1 on closed→open and new-detail pushes without resetting level 0; contain terminal wheel/touch/programmatic scroll; preserve animation, held detail, focus/inert, regular layout, BottomSheet API, and one content instance. `BottomSheet.tsx` is unclaimed |

The active five-path remaining-intent row above is the only Iconography ownership created
by this table. Item Detail/Get-It is released. Market Details remains a separate disjoint
exact two-path retained claim; its NavigationStack correction is a disjoint two-path
Motion claim.
...
The controller does not own product implementation by default. The completed
**Identity & Email Delivery — branded Neon Auth OTP** exception is released and grants
no continuing source or package ownership. It does not widen the controller's standing
role. The controller may make a path-scoped local documentation commit after an
...
Founder/CEO checkpoint-review request plus every documented gate passing at the exact
candidate commit.
...
- The reported `searchItems()` schema/provenance defect is statically corrected at
  `c6f304b`, and deployed-current-main `97b74af` passed its strengthened disposable SQL
  unavailable/contradictory cases plus exact cleanup. The pathless gate is closed.
- T2D SEO isolation, the T2C search correction, and Search disposable execution are
  complete. Shared-target `0009`/`0010`/`0011` compatibility, write safety, account
  deletion provider capability, automated release gates, seller contact, and pilot
...
and do not fabricate a ledger receipt. Forward repair `0012` is source/disposable complete
at `d5991c9`; its pathless shared-rollout gate is recorded below.

Presence work must proceed in this dependency order:

`containment [complete] -> ADR-016 acceptance [complete] -> corrected-unapplied 0012 source/disposable validation [complete; paths released] -> exact-target identity/role/scheduler proof [sole pathless blocker] -> shared rollout under recorded approval -> server boundary -> consent/lifecycle -> snapshot lifecycle -> map DTO/UI -> independent refutation -> separately released pilot`
...
by accepted `c960860`. Corrected-unapplied `0012` source and disposable proof completed at
`d5991c9`, and all four paths are released. The remaining dependency is pathless exact
target identity, role, and scheduler proof before shared rollout under the recorded
approval, not app/UI/map implementation authority.
...
- Forward `0012` source and disposable validation completed at `d5991c9`; both presence
  pillars, `0012_guarded_presence.sql`, and its focused contract are released. The shared
  database remains untouched. Shared rollout is blocked solely on pathless proof of exact
  target identity, execution role, and scheduler capability/configuration. Contribution
  integrity must follow as `0013` or later.
...
|---|---|---|---|---|
| **G0 governance reconciliation** | event-driven employee upkeep, docs by default | Quality & Release Controller | Reconcile checkpoint evidence, ownership, gates, and employee handoffs only when explicitly assigned | Standing ownership remains `LANES.md` and `docs/architecture/RELEASE-CONTROLLER.md`; the completed branded Neon Auth OTP implementation and embedded-logo follow-up paths are released, with no continuing app/package/schema/migration authority |
...
| **P0 search schema + provenance repair** | deployed-current-main `97b74af` disposable SQL PASS; strengthened unavailable/contradictory cases and exact cleanup passed; gate closed and released | persistent Search & Ranking Engineer; independent disposable-DB refuter | Preserve evidence that search price/count/time/availability derive from admissible observed context without synthetic/partner/reference/inferred leakage | Owns no code paths or continuing DB gate; no schema/migration rewrite, shared DB, ranking expansion, category work, push, or deployment follows |
...
| **Contribution containment and integrity** | controller runtime PASS; exact five paths released | Contribution Integrity & Moderation Engineer; independent static/source and runtime evidence | Existing fail-closed containment plus a completed, unauthorized `0013+` planning packet covering ADR-019, schema/migration/manifest, app/ops paths, and dependencies | Runtime containment is closed. `actions.ts`, `page.tsx`, both contribution sheets, and `strings.ts` are released; only `page.tsx` later transferred to retained Market Details. The `0013+` packet remains planning-only behind separate Presence/shared-migration and activation gates. No DB/push/deploy |
| **Market Details — Place Offer Surface Redesign** | ACTIVE/RETAINED exact two-path slice; `8b0d5e4` static PASS, runtime REFUTED | persistent Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; independent visual/accessibility and source/data refuters | `src/app/page.tsx`; new `src/design-system/components/PlaceOfferRow.tsx` | Runtime refutation found stale level-1 open position and terminal scroll escaping the document. Preserve truth, imagery/fallback, pinned Get it, states, compact/desktop, light/dark, 44px, and fulfilment exclusion. `actions.ts` is released; self-location/avatar and H38 wait behind `page.tsx`; H31 stays unclaimed |
| **Market Details level-1 scroll reset and containment** | ACTIVE exact two-path disjoint Motion sub-lane | Motion & Interaction Engineer; fresh independent refuter | `src/design-system/components/NavigationStack.tsx`; `scripts/motion-contracts.test.ts` | Reset level 1 on closed→open/new detail without resetting level 0; prevent terminal wheel/touch/programmatic chaining; preserve push/pop animation, held detail, focus/inert, regular layout, BottomSheet API, and one content instance. Do not claim `BottomSheet.tsx` |
...
| **Identity & Email Delivery — branded Neon Auth OTP** | completed/released at `64d3519` + `8583495`; monochrome `d549d42` and embedded-logo `712719f` production VERIFIED | current controller implementation; independent default-to-refuted reviewers | Released paths: `src/app/api/webhooks/neon-auth/route.ts`; `src/lib/auth-email.ts`; `src/lib/auth-email-logo.ts`; `scripts/auth-email-contracts.test.ts`; `docs/operations/AUTH-EMAIL-DELIVERY.md`; `package.json`; `package-lock.json`; `public/logo.png` remains unclaimed and is not a runtime email dependency | Prior delivery and monochrome evidence remains recorded above. `712719f` embeds the fixed 96×96 real monochrome PNG through CID `wetindey-logo@auth.wetindey.live` with no remote URL; the focused contract validates PNG structure/CRC/decompression/filters/grayscale/SHA plus CID-only HTML and inline metadata. Focused PASS; independent final VERIFIED no P1/P2/P3. Deployment `dpl_B7QwNqAYTToHsAXsVCM5jh1JXwRM` is Ready/aliased; fresh OTP `200`; Gmail confirmed unread `wetindey-logo.png`, 3334 bytes, exact CID, `has_attachment=false`. All three follow-up paths are released. Process-local caches, cross-instance Message-ID limits, provider Beta, SPF/DKIM/DMARC monitoring, and email-client rendering remain operational follow-up, not path claims |
...
| **Account deletion reconciliation** | provider proof REFUTED on exact current Neon target; Founder/legal policy approved; pathless and owns no app paths | provider/platform configuration owner, then a later app-owned orchestration/UI lane | Authenticated fresh-session `POST /delete-user` returns `404`; unauthenticated returns `401`, proving the route is mounted but `user.deleteUser.enabled` is false | The disposable identity required emergency SQL cleanup; that cleanup is not self-delete evidence. Remaining blockers are provider capability/configuration and later exact app orchestration/UI ownership, destructive-flow safeguards, retention behavior, direct evidence, and release sequencing |
...
| **Human Interface: category + Aboki FX flattening** | scoped visible behavior PASS; completed paths released; combined release evidence still REFUTED on residual gaps | HI Quality employee refuter / Quality & Release Controller evidence custody | Visible selector/Aboki behavior passed desktop/compact and light/dark | The flattening lane released `CategorySelectorSheet.tsx` and its former `ExchangePanel.tsx` claim; Safari keyboard focus plus artificial loading/error/cache/empty states remain unproven release-evidence gaps. `ExchangePanel.tsx` is now owned only by the active Iconography intent-adoption lane |
...
| **Controlled Semantic Iconography bounded expansion** | foundation released; Item Detail/Get-It `ed5a6cb` static+runtime PASS/released; remaining intent adoption ACTIVE exact five paths | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac`; independent visual/accessibility refuters | Active: `SettingsSheet.tsx`, `SheetPicker.tsx`, `CurrencyPickerSheet.tsx`, `ExchangePanel.tsx`, `MapLoader.tsx`. Foundation and Item Detail/Get-It paths released | Apply ADR-018 contextual dimensional solid orbs, neutral universal utilities, unchanged authentic flags, and no copy/data/actions/interactions changes. Require light/dark/grayscale/forced-colors/reduced-motion/44px/AX refutation. No page/actions/map/DB |
...
| **Shared ModalSheet focus hardening** | completed at `f19990b`; Safari refuter PASS; ModalSheet released and contract later transferred | Motion & Interaction Engineer / independent Safari refuter | Released: `src/design-system/components/ModalSheet.tsx`; transferred: `scripts/motion-contracts.test.ts` | Zero-focusable initial/Tab/Shift+Tab containment passes Safari refutation while preserving Escape, Close, restoration, nested/pushed sheets, motion, stacking, and layout. The contract path is now owned only by the disjoint Market Details scroll-containment sub-lane |
...
| **Corrected-unapplied Nearby Presence `0012`** | source/disposable validation complete at `d5991c9`; exact four paths released; shared rollout pathless/blocked | Presence Platform Engineer `019f759f-3521-7ee1-90a3-5af3539d757e`; independent new-cluster refuter | Released: `src/db/pillars/80-presence-services.sql`; `src/db/pillars/90-presence-security.sql`; `src/db/migrations/0012_guarded_presence.sql`; `scripts/presence/presence-migration-contract.test.ts` | The five SQL failures, deterministic idle cleanup, deletion purge/anonymization, blank+upgrade lineages, nine RPCs, lifecycle/security/cleanup/idempotence, and new-cluster validation are complete. Shared rollout remains blocked solely on exact target identity, execution role, and scheduler proof; no DB action or source claim follows |
| **Nearby Presence / Remote Sharing privacy policy** | Presence seven-path proposal wholly unclaimed behind active Market Details ownership of `page.tsx`; Remote Sharing policy-only and unclaimed | Presence Platform Engineer `019f759f-3521-7ee1-90a3-5af3539d757e` waiting | Nearby Presence stays fresh-GPS, reciprocal, and area-bound. Consent to share with someone outside the physical area is a separate Remote Sharing mode, never an override. Safe default: explicit trusted audience, coarse area/country label, short expiry, immediate revoke, no background tracking, and no automatic exact-location disclosure | Global “any signed-in user” visibility is a separate high-risk privacy/legal gate and is not authorized. Peer presence remains fail-closed through `sharedUsers={[]}`. Exact Presence path statuses and proof gates are recorded below; no product path claimed |
...
| `src/app/_components/ExchangePanel.tsx` | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac` | **Iconography intent adoption — remaining sheets and MapLoader** — active exact five-path claim; prior provider-aware implementation is complete/released |
| `src/app/page.tsx` | ~1300 | **Market Details — Place Offer Surface Redesign** — active/retained exact two-path slice owned by Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; `8b0d5e4` static PASS, compact runtime REFUTED. Self-location/avatar and H38 wait pathless for release |
...
| `src/design-system/components/NavigationStack.tsx`, `scripts/motion-contracts.test.ts` | — | **Market Details level-1 scroll reset and containment** — active exact two-path Motion claim; `BottomSheet.tsx` remains unclaimed |
| `src/app/_components/ReportPriceSheet.tsx`, `src/app/_components/ConfirmVisitSheet.tsx` | — | **released** after controller runtime artifact closed Contribution containment with PASS |
...
| `src/app/reports/security/csp/route.ts`, `src/app/reports/security/csp/cleanup/route.ts`, `src/lib/security/csp-report.ts`, `src/lib/security/csp-report-store.ts`, `.env.example`, `vercel.json`, `scripts/csp-report-contracts.test.ts` | — | **released** after CSP collector operations PASS; ongoing provider/WAF/cron/store observation is pathless and creates no source claim |
| `src/middleware.ts`, `src/lib/seo.tsx`, `scripts/csp-policy-contracts.test.ts` | — | **released clean** after Preview `dpl_DXZqqwy3srE6E6Ju8XLJ968otyf7` refuted nonce propagation at the upstream Next/Vercel adapter boundary; no identical source experiment without new adapter/version evidence |
...
| `src/app/_components/ItemDetailSheet.tsx`, `src/app/_components/GetItSheet.tsx` | — | **released** after `ed5a6cb` static PASS and combined runtime PASS |
| `src/app/_components/SettingsSheet.tsx`, `src/design-system/components/SheetPicker.tsx`, `src/app/_components/CurrencyPickerSheet.tsx`, `src/app/_components/ExchangePanel.tsx`, `src/design-system/components/MapLoader.tsx` | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac` | **Iconography intent adoption — remaining sheets and MapLoader** — active exact five-path claim; ADR-018 visual/AX contract only, no copy/data/actions/interaction changes |
...
| `src/app/currency-actions.ts`, `src/app/_data/reference-currencies.ts`, `src/app/_components/CurrencyPickerSheet.tsx`, `src/app/_components/CurrencyFlag.tsx` | — | Provider-aware implementation `ed9483c` + `a4eaed3` is complete; `currency-actions.ts`, reference currencies, and `CurrencyFlag.tsx` remain released, while `CurrencyPickerSheet.tsx` is transferred exclusively to the active Iconography intent-adoption lane |
...
| **Human Interface: category + Aboki FX flattening** | Human Interface employee; HI Quality refuter; Quality & Release Controller evidence custody | ✅ scoped visible PASS; completed paths released; combined release verdict remains REFUTED | — | Founder-directed Human Interface correction | 2026-07-18 | Selector/Aboki behavior passed visible desktop/compact and light/dark evidence. Safari keyboard focus and artificial loading/error/cache/empty states remain unproven. Those are residual release-evidence gaps, not retained implementation ownership. `CategorySelectorSheet.tsx` remains released; `ExchangePanel.tsx` is now owned only by the active Iconography intent-adoption lane. |
...
| **Contribution containment plan** | Contribution Integrity & Moderation Engineer `019f75a3-f50d-7180-8e92-0a7aabd8a98c`; independent static/source refuter; HI Quality browser refuter | ✅ controller runtime artifact PASS; exact five paths released | — | Immediate contribution safety containment | 2026-07-18 | The runtime artifact closes containment. `src/app/actions.ts`, `src/app/page.tsx`, both contribution sheets, and `src/core/i18n/strings.ts` are released; only `page.tsx` later transferred to retained Market Details. The `0013+` planning packet remains unauthorized behind separate Presence/shared-migration and activation gates. |
...
| **Market Details — Place Offer Surface Redesign** | Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; independent visual/accessibility and source/data refuters | 🟠 active/retained exact two-path slice; `8b0d5e4` static PASS, runtime REFUTED | `src/app/page.tsx`; new `src/design-system/components/PlaceOfferRow.tsx` | Founder-rejected live market detail | 2026-07-18 | Runtime found the level-1 scroller opens stale and terminal scroll escapes the document. Preserve truth, imagery/fallback, pinned Get it, states, compact/desktop, light/dark, 44px, and fulfilment exclusion. `actions.ts` is released; self-location/avatar and H38 wait behind `page.tsx`; H31 remains unclaimed. |
| **Market Details level-1 scroll reset and containment** | Motion & Interaction Engineer; fresh independent refuter | 🟢 active exact two-path disjoint sub-lane | `src/design-system/components/NavigationStack.tsx`; `scripts/motion-contracts.test.ts` | Compact Market Details runtime correction | 2026-07-18 | Reset persistent level 1 on closed→open/new detail without resetting level 0; contain terminal wheel/touch/programmatic scroll; preserve animation, held detail, focus/inert, regular layout, BottomSheet API, and one content instance. `BottomSheet.tsx` is unclaimed. |
...
| **Account deletion provider evidence** | Quality & Release Controller evidence custody | 🔴 exact-current-Neon provider proof REFUTED; Founder policy approved; pathless | — | Identity/privacy provider gate | 2026-07-18 | Authenticated fresh-session `POST /delete-user` returns `404`; unauthenticated returns `401`, proving the route is mounted but `user.deleteUser.enabled` is false. The disposable identity was emergency SQL-cleaned; that is not self-delete evidence. Provider capability/configuration remains blocked, followed later by a separately claimed app-owned orchestration/UI lane. No app path or deletion completion is claimed. |
...
| **Corrected-unapplied Nearby Presence `0012`** | Presence Platform Engineer `019f759f-3521-7ee1-90a3-5af3539d757e`; independent new-cluster refuter | ✅ source/disposable validation complete at `d5991c9`; exact four paths released; shared rollout pathless | — | ADR-016 schema / forward `0012` | 2026-07-18 | The five SQL defects, blank+upgrade lineages, all nine RPCs, lifecycle/security/cleanup/idempotence, deterministic idle retention, account-deletion purge/anonymization, and independent disposable evidence are complete. The shared database remains untouched; rollout is blocked solely on exact target identity, execution role, and scheduler proof. |
...
| **`page.tsx` modularisation and reusable core engines** | future architecture owner; independent boundary/refutation reviewer required | ⚪ pathless architecture plan only; Market Details owns `src/app/page.tsx` | — | ADR-002 correctness-before-boundaries | 2026-07-18 | Deliverables are a current code tree, live caller/dependency graph, ownership-boundary matrix for discovery/search, location, map/presence, sheet/presentation, contribution, exchange, seller, trust/confidence, and legal/analytics, plus an incremental strangler sequence and documentation updates. Each extraction needs exact paths, live wiring in the same change, no behavior regression, and independent refutation. Prefer shared typed contracts/configuration and reusable core engines; keep domain logic out of UI. `src/modules` remains aspirational until wired to live callers in the same change. |
...
| **Nearby-user Mapbox popup follow-up** | Antigravity (External Approved Contractor - Product Engineering & UX Department) | ⚪ deferred behind presence dependency chain; no popup/peer claim | — | ADR-016 map DTO/UI stage | 2026-07-18 | `src/integrations/maps/MapboxAdapter.ts` is released after H37 `6611068`, but that creates no popup/peer claim. Nearby-user UI requires a fresh exact handoff after its dependencies. Existing public name/avatar/contact, `Live Contributor`, and direct-contact behavior remain unverified and must not ship. |
...
| **P0 search schema + provenance repair** | persistent Search & Ranking Engineer with disposable verifier Beauvoir | ✅ deployed-current-main `97b74af` disposable SQL PASS; pathless execution gate closed/released | — | Food Truth & Pilot Operations / Stage 0 | 2026-07-18 | Strengthened unavailable and contradictory cases plus exact cleanup passed. No code/schema path or continuing database gate remains claimed. Search & Ranking remains persistent and available; this evidence grants no shared-target migration, push, or deployment authority. |
...
place-marker accessible names and is independently NOT REFUTED; both Maps paths are
...
to a future freshly claimed **map** lane and creates no current path ownership.
...
| **H37 semantic market-marker handoff** | Linnaeus read-only audit; completed by `6611068` | ✅ implementation independently NOT REFUTED; both paths released | — | Historical design handoff only | Current HEAD carries `placeType` through the canvas and adapter and renders semantic symbols with accessible labels and fallback. `sharedUsers={[]}` / `getSharedUserLocations()=[]` leave peer avatars non-live; self-avatar and any contextual-orb/selected-glow follow-up are separate unclaimed slices. |
...
claimed for self or peers. Independent review returned NOT REFUTED for `6611068`, and both
place-marker paths are released. Contextual-orb/selected-glow follow-up is not claimed
complete; this historical H37 record grants no edit, browser, or DB authority.
...
| `src/app/page.tsx` | **Conflict:** exclusively owned by the active Market Details slice under Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`. |
| `src/design-system/components/MapboxCanvas.tsx` | Released after H37 `6611068`; available only through a fresh atomic claim. Self and peer work remain separate and unclaimed. |
| `src/integrations/maps/MapboxAdapter.ts` | Released after H37 `6611068`; available only through a fresh atomic claim. No Presence, self/avatar, peer, or camera authority transfers. |
...
Presence must wait until Market Details releases `src/app/page.tsx`, then request a fresh
...
Corrected-unapplied `0012` source and disposable validation completed at `d5991c9`.
The two presence pillars, `0012_guarded_presence.sql`, and the focused contract are
released. Blank and upgrade lineages, all nine RPCs, lifecycle/security,
idle-retention cleanup, account-deletion purge/anonymization, idempotence, and independent
new-cluster evidence are complete. The shared database remains untouched; rollout is
blocked solely on pathless exact target identity, execution role, and scheduler
capability/configuration proof. No migration, pilot traffic, source ownership, push, or
...
The former Motion diagnosis worker is no longer addressable. Mendel (`019f7638-4f36-7952-b775-e9b141f0fd7d`) diagnosed the shared lifecycle defect; Carver implemented `f7cddba`, Leibniz refuted disabled-fieldset ancestry, and Godel corrected it in `9fe9d4e`. Candidate `3e28c63` established the shared visibility predicate. Final correction `f19990b` completed the zero-focusable containment over `ModalSheet.tsx` and its focused contract; the Safari refuter returned PASS. `ModalSheet.tsx` remains released, while `scripts/motion-contracts.test.ts` transferred to the active disjoint Market Details scroll-containment lane. The five Contribution paths remain protected.
...
Hegel (`019f7645-cc25-7d42-b537-5a0eb37cda69`) confirmed the current market detail at `src/app/page.tsx` drops price range, availability, freshness, last-observed, image, and provenance semantics, and uses the artificial `max-h-[40vh]` dead zone. Commit `8b0d5e4` passed static refutation, but compact runtime refuted it because the persistent level-1 scroller opens stale and terminal scroll escapes to the document. The persistent Human Interface Design Engineer retains only `src/app/page.tsx` and new `src/design-system/components/PlaceOfferRow.tsx`; `src/app/actions.ts` is released. Preserve the pinned `Get it` region, truthful read-side provenance, image fallback, empty/loading/error states, compact/desktop, light/dark, 44px targets, and fulfilment exclusion. The disjoint Motion correction owns only `NavigationStack.tsx` and `scripts/motion-contracts.test.ts`; no `BottomSheet.tsx`, generic category contract, DB, migration, push, or deployment work is authorized.
...
Market Details remains active/retained for the persistent Human Interface Design Engineer
over only `src/app/page.tsx` and new
`src/design-system/components/PlaceOfferRow.tsx`. Its runtime-refuted scroll containment
is assigned separately to Motion over `NavigationStack.tsx` and
`scripts/motion-contracts.test.ts`. H38, self-location/avatar, and Presence UI wait
pathless for `page.tsx`; H31 remains queued and unclaimed on released `actions.ts`.
...
| Human Interface / Iconography | Solid semantic icon orbs, sheet hierarchy, category trigger, market-detail visual system, and accessibility refutation | Foundation and `ed5a6cb` Item Detail/Get-It adoption passed/released; exact five-path remaining intent-adoption lane active under the persistent Iconography Lead |
...
| Presence / Safety Engineering | Consent-based nearby presence, trusted people, blocks, reports, leases, and pilot gates | Corrected-unapplied `0012` source/disposable validation completed at `d5991c9`; four paths released. Shared DB untouched and rollout blocked solely on exact target identity/role/scheduler proof; seven app/UI/map paths remain unclaimed |
...
1. **Market Details — Place Offer Surface Redesign**: active/retained exact two-path slice
   for the persistent Human Interface Design Engineer over `src/app/page.tsx` and new
   `src/design-system/components/PlaceOfferRow.tsx`. `8b0d5e4` passed static refutation but
   runtime refuted stale level-1 reopening and terminal document scroll escape. Preserve
   truth, trust, imagery, pinned Get-It, accessibility, and fulfilment boundaries.
   Self-location/avatar and H38 wait behind `page.tsx`; H31 is queued/unclaimed on released
   `actions.ts`.
2. **Market Details level-1 scroll reset and containment**: active exact two-path Motion
   sub-lane over `src/design-system/components/NavigationStack.tsx` and
   `scripts/motion-contracts.test.ts`; `BottomSheet.tsx` remains unclaimed.
3. **Semantic iconography**: foundation and `ed5a6cb` Item Detail/Get-It adoption are
   PASS/released. The persistent Iconography Lead now actively owns only Settings,
   SheetPicker, CurrencyPicker, ExchangePanel, and MapLoader for ADR-018 intent adoption;
   light/dark/grayscale/forced-colors/reduced-motion/44px/AX refutation is required.
4. **Map Symbols, self location, and Nearby Presence**: H37 semantic symbols are complete
   and both Maps paths are released. The named self-location/avatar vertical remains
   unclaimed until Market Details releases `src/app/page.tsx` and a fresh handoff lands.
   Peer UI, contextual-orb/selected-glow follow-up, and camera changes remain separate and
   unclaimed. Presence `0012` source/disposable work is complete; shared rollout remains
   pathless behind exact target identity/role/scheduler proof. Never use simulated centres
   to unlock real peers.
5. **Seller Onboarding and Reusable RBAC**: Seller Platform / Identity & Access;
   architecture first, then exact server/UI/schema claims with security and
   privacy refutation.
6. **Aboki FX**: Currency / Money Experience; follow ADR-017 and keep provider
   attribution, flags, catalog intersection, reverse conversion, and no
   transaction-shaped language.
7. **CSP Reporting and production nonce P1**: Collector operations PASS and its source
   paths are released; monitoring remains pathless, fail-open, and report-only, with
   per-region counters as the residual. The nonce P1 remains pathless at the upstream
   Next/Vercel adapter boundary. Enforcement is prohibited.
8. **Food Truth & Pilot Operations**: Trust, Quality, Operations, and Seller
