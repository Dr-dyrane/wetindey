...
| **governance / phase-6** | this session | 🟢 active | `AGENTS.md`, `DECISIONS.md`, `WETINDEY_BIBLE.md`, `docs/adr/**`, `docs/architecture/**`, `docs/USER-FLOW.md`, `docs/APP-MAP.md`, `LANES.md` | Phase 6 | 2026-07-16 | ADR-001/002/003 landed. **Now working Phase 6 — amend the constitution to the code.** Four decisions the Bible lists *open* have shipped and are load-bearing (Mapbox, Drizzle, Neon Auth, freshness windows) — an agent reading "ORM — open" may rewrite working code. Also killing `USER-FLOW.md`'s delivery lines (ADR-001) and its stale "buttons are inert" table. **Phase 6 is subtraction:** record what shipped, delete what is false, write nothing new. Touching no source code. |
