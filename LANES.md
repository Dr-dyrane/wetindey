...
It changes nothing about THE ONE REMAINING GATE above. Going live still means applying migrations 0017/0018/0019 to Preview then Production and flipping the fail-closed flags, and that apply requires the shared-database migration-owner credential, run by the Founder or a named executor under the recorded disposable-then-Preview-then-Production proof with independent refutation. The controller cannot hold or use that credential and cannot mutate a shared or production database; it has prepared everything up to that key. Blanket approval authorizes proceeding, it does not substitute for the physical apply. Next operational step is unchanged: the Founder runs the credentialed apply with the controller-supplied runbook, or names the executor who holds the credential.
