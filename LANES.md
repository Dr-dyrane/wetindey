...
#### Docs-truth conformance wave 2: SERVICE-ARCHITECTURE amendment - ACTIVE

Owner: Private Contractor, Maps Delivery `c9c17443-ef5e-4a7b-9b6e-c8f5381da30c`. The architecture of record self-declares that where it and the code disagree, it must be corrected; six of its present-tense claims are now false and one is actively dangerous (the "wire getOfferTrustBatch first, it is the whole fix for R2" directive would resurrect a public endpoint deliberately removed at 8fbd259). Following the document's own AMENDED convention (as ADR-006 used): an amendment block correcting the trust-orphan claims, the god-file description (page.tsx is 12 lines, actions split across src/app/_actions), the selectedPlaceId never-used claim, the i18n single-consumer claim, and the three-state-libraries claim (xstate and jotai are gone), each pinned to its correcting commit. Exact paths: `docs/architecture/SERVICE-ARCHITECTURE.md` only. Completion: every amendment claim verified against tree and history, decision prose untouched outside the amendment block and per-claim strike-marks, independent default-to-REFUTED refutation, path-scoped commit, release.
