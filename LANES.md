...
| Motion & Interaction Engineer | Available, no active path claim | ModalSheet focus containment completed at `f19990b` with Safari refuter PASS; both paths are released |
...
| Contribution containment/integrity (`495750a`) | **Closed / released with controller runtime PASS** | The runtime artifact closes containment. `src/app/actions.ts`, `src/app/page.tsx`, `ReportPriceSheet.tsx`, `ConfirmVisitSheet.tsx`, and `src/core/i18n/strings.ts` are released; the first two transfer only through the active Market Details claim below |
| Modal zero-focusable/focus restoration | **Closed / released with Safari PASS** | Commit `f19990b` is on `origin/main`; Safari refutation passed and both `ModalSheet.tsx` and `scripts/motion-contracts.test.ts` are released |
...
| Market Details — Place Offer Surface Redesign | **Active, exact three-path atomic slice** | Persistent Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d` owns only `src/app/page.tsx`, `src/app/actions.ts`, and new `src/design-system/components/PlaceOfferRow.tsx`. Self-location/avatar, H38, and H31 wait pathless for this short slice to release |
...
Founder. Remove the artificial `max-h-[40vh]` dead zone; preserve a pinned `Get it`; add
...
Do not widen into `ItemCard`, `ListRow`, `IconOrb`, self-location/avatar, H38, H31,
Presence, schema, migrations, fulfilment, or another category abstraction. Self-location/
avatar, H38, and H31 remain pathless and wait for this slice to release its two live paths.
...
**PASS**. Both exact paths are released; no caller, page, action, database, map, or
iconography ownership was created.
...
| **Contribution containment and integrity** | controller runtime PASS; exact five paths released | Contribution Integrity & Moderation Engineer; independent static/source and runtime evidence | Existing fail-closed containment plus a completed, unauthorized `0013+` planning packet covering ADR-019, schema/migration/manifest, app/ops paths, and dependencies | Runtime containment is closed. `actions.ts`, `page.tsx`, both contribution sheets, and `strings.ts` are released; only `actions.ts` and `page.tsx` transfer to active Market Details. The `0013+` packet remains planning-only behind separate Presence/shared-migration and activation gates. No DB/push/deploy |
| **Market Details — Place Offer Surface Redesign** | ACTIVE exact three-path short atomic slice | persistent Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; independent visual/accessibility and source/data refuters | `src/app/page.tsx`; `src/app/actions.ts`; new `src/design-system/components/PlaceOfferRow.tsx` | Preserve truthful price/unit/availability/freshness/provenance, imagery/fallback, pinned Get it, states, compact/desktop, light/dark, 44px, and fulfilment exclusion. Self-location/avatar, H38, and H31 wait pathless for release |
...
| **Shared ModalSheet focus hardening** | completed at `f19990b`; Safari refuter PASS; exact two paths released | Motion & Interaction Engineer / independent Safari refuter | `src/design-system/components/ModalSheet.tsx`; `scripts/motion-contracts.test.ts` | Zero-focusable initial/Tab/Shift+Tab containment now passes Safari refutation while preserving Escape, Close, restoration, nested/pushed sheets, motion, stacking, and layout. No current ownership; Contribution's completed five-path containment remains released |
...
|---|---:|---|
| `src/app/actions.ts` | ~1358 | **Market Details — Place Offer Surface Redesign** — active exact three-path atomic slice owned by Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; no H31, Presence, Contribution, or other widening |
...
| `src/app/page.tsx` | ~1300 | **Market Details — Place Offer Surface Redesign** — active exact three-path atomic slice owned by Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`. Self-location/avatar and H38 wait pathless for release |
| `src/design-system/components/ModalSheet.tsx`, `scripts/motion-contracts.test.ts` | — | **released** after `f19990b` and Safari refuter PASS for zero-focusable focus containment |
...
| **Contribution containment plan** | Contribution Integrity & Moderation Engineer `019f75a3-f50d-7180-8e92-0a7aabd8a98c`; independent static/source refuter; HI Quality browser refuter | ✅ controller runtime artifact PASS; exact five paths released | — | Immediate contribution safety containment | 2026-07-18 | The runtime artifact closes containment. `src/app/actions.ts`, `src/app/page.tsx`, both contribution sheets, and `src/core/i18n/strings.ts` are released; page/actions transfer only to active Market Details. The `0013+` planning packet remains unauthorized behind separate Presence/shared-migration and activation gates. |
| **Shared ModalSheet focus hardening** | Motion & Interaction Engineer; independent Safari refuter | ✅ final `f19990b`; Safari PASS; both paths released | — | Shared modal accessibility | 2026-07-18 | Zero-focusable initial/Tab/Shift+Tab containment passed Safari refutation while preserving Escape, Close, restoration, nested/pushed sheets, motion, stacking, and layout. Completed exactly `src/design-system/components/ModalSheet.tsx` and `scripts/motion-contracts.test.ts`; no current ownership. |
| **Market Details — Place Offer Surface Redesign** | Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; independent visual/accessibility and source/data refuters | 🟢 active exact three-path atomic slice | `src/app/page.tsx`; `src/app/actions.ts`; new `src/design-system/components/PlaceOfferRow.tsx` | Founder-rejected live market detail | 2026-07-18 | Preserve truthful price/unit/availability/freshness/provenance, imagery/fallback, pinned Get it, states, compact/desktop, light/dark, 44px, and fulfilment exclusion. Self-location/avatar, H38, and H31 wait pathless for release. |
...
Descartes (`019f7644-8b2a-7910-8221-533a6d7afac1`) confirmed `getCoverageForPoint` still orders only by distance, so coincident active LGA/neighbourhood centroids rely on unspecified PostgreSQL row order. The bounded correction is one query-only change in `src/app/actions.ts`: retain distance first, prefer `areas.type = 'neighborhood'`, then `areas.slug ASC`. Implementation waits pathless behind the short Market Details slice; no seed, live DB, migration, UI, push, or deployment change is authorized. Acceptance requires static SQL refutation plus a controlled coincident-area fixture after `actions.ts` is released.

### Market detail offer surface handoff

Hegel (`019f7645-cc25-7d42-b537-5a0eb37cda69`) confirmed the current market detail at `src/app/page.tsx` drops price range, availability, freshness, last-observed, image, and provenance semantics, and uses the artificial `max-h-[40vh]` dead zone. The persistent Human Interface Design Engineer now owns the coordinated atomic claim over `src/app/page.tsx`, `src/app/actions.ts`, and new `src/design-system/components/PlaceOfferRow.tsx`; do not modify `ItemCard`, `ListRow`, or `IconOrb`. Preserve the pinned `Get it` region, truthful read-side provenance, image fallback, empty/loading/error states, compact/desktop, light/dark, 44px targets, and fulfilment exclusion. No new generic category contract, DB, migration, push, or deployment work is authorized.

CEO sequencing checkpoint: the iconography foundation and `ed5a6cb` Item Detail/Get-It
adoption passed and released; the disjoint five-path remaining-intent lane is active. Market Details is now active for
the persistent Human Interface Design Engineer over only `src/app/page.tsx`,
`src/app/actions.ts`, and new `src/design-system/components/PlaceOfferRow.tsx`. H31, H38,
self-location/avatar, Presence UI, and any later query/data contract wait pathless until
this short atomic slice releases the relevant live paths.
...
1. **Market Details — Place Offer Surface Redesign**: active exact three-path atomic slice
   for the persistent Human Interface Design Engineer over `src/app/page.tsx`,
   `src/app/actions.ts`, and new `src/design-system/components/PlaceOfferRow.tsx`.
   Preserve truth, trust, imagery, pinned Get-It, accessibility, and fulfilment boundaries.
   Self-location/avatar, H38, and H31 wait pathless.
2. **Semantic iconography**: foundation and `ed5a6cb` Item Detail/Get-It adoption are
...
3. **Map Symbols, self location, and Nearby Presence**: H37 semantic symbols are complete
...
6. **CSP Reporting and production nonce P1**: Collector operations PASS and its source
