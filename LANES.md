...
#### Image delivery repairs: review avatars and item hero - ACTIVE

Owner: Private Contractor, Maps Delivery `c9c17443-ef5e-4a7b-9b6e-c8f5381da30c`, from discovery sweep three. P1: reviewer avatars are CSP-blocked in production today, the app's only raw img tag loads a Vercel Blob URL whose host neither CSP's img-src allows, rendering a blank circle with the initials fallback unreachable and feeding noise into the app's own CSP report store; the fix routes through next/image (the host is already in remotePatterns, so delivery becomes 'self' and NO CSP edit is needed, keeping the Founder-gated policy untouched) with a working error fallback. P2: the SEO item page's Wikimedia hero is the one image still on the optimizer path all three sibling surfaces avoid with unoptimized for the documented server-side 429 risk, and no image anywhere carries priority, the likely source of the recurring LCP console warning; the hero gains both. Riders: tailwind.config.ts drops two dead content globs (src/pages, src/modules); public/offline.html's title drops its em dash per the house rule. Routed to controller, not claimed: function region pinning (no regions in vercel.json, prod observed serving Lagos from iad1 with documents uncacheable by design; the right region depends on where Neon lives). Exact paths: `src/app/_components/get-it-sheet/views/GetItReviewsView.tsx`; `src/app/item/[slug]/page.tsx`; `tailwind.config.ts`; `public/offline.html`; new `scripts/perf/image-delivery-contract.test.ts` (registered in the static stage via `scripts/quality/run-stage.mjs`). Completion: contract green in the stage, gates green, a browser proof of the avatar path rendering via /_next/image, independent default-to-REFUTED refutation, path-scoped commit, release.
