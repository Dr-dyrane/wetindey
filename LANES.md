...
| Human Interface Design Engineer | Active exact one-file Place Offer Row availability semantics | Prior edge-bleed runtime passed; Founder color refutation reactivates only `src/design-system/components/PlaceOfferRow.tsx` |
...
| Market Details — Place Offer Row availability semantics | **Active, exact one-file HI/Iconography lane** | `src/design-system/components/PlaceOfferRow.tsx` only. Available/confirmed uses `status-confirmed`; unavailable uses `status-unavailable`; uncertain/stale uses `status-caution` when represented; neutral only for genuinely unknown. Preserve explicit text/icon so color is not sole signal, light/dark/grayscale/forced-colors, and all data/copy/actions |
