...
#### Auth-email injection-guard contract hardening - ACTIVE

Owner: Private Contractor, Maps Delivery `c9c17443-ef5e-4a7b-9b6e-c8f5381da30c`, from a read-only auth/email sweep that found the subsystem otherwise clean (no live defect; the email path escapes user data, ships a text/plain alternative, has no callback URLs, and keeps every secret off the client). The one bounded gap: the anti-injection guards are real but unpinned, so they can regress green. `escapeHtml` on the OTP code (auth-email.ts) is never exercised with a special character because `verifyNeonOtpWebhook` validates `^\d{6}$` first, and the recipient-email and from/replyTo CRLF rejections have no contract case. This lane adds cases exercising each guard directly (render with an injection-shaped code asserting the output carries no raw `<`; a malicious or oversized or CRLF recipient email rejected; the from/replyTo CRLF guard triggered), so a future loosening of the numeric validator cannot silently drop the last defense. Scripts-only, no runtime change. Exact paths: `scripts/auth-email-contracts.test.ts` only. Completion: contract green with the new cases, each new assertion mutation-proven against the guard it pins, gates green, independent default-to-REFUTED refutation, path-scoped commit, release.
