...
|---|---|---|---|
| **Semantic iconography foundation** | 🔴 `1d051558` REFUTED on one P2; exact five paths retained for forward correction/refutation | Iconography & Visual Systems Lead implementation owner; independent visual/accessibility refuter | Forward correction is limited to `src/app/globals.css` and `scripts/iconography-contracts.test.ts`: remove the forced-colors `CanvasText` override so glyphs inherit parent system colour, and make the contract prohibit glyph-level forced-colour ownership rather than self-attesting licensing prose | No P1. Sheet Adoption, Market Details, and Map Symbols remain closed pending a path-scoped forward commit and no-P1/P2 re-refutation |
...
| **H27 seed-trust correction** | controller-owned bounded Catalog Stewardship worker; fresh independent static refuter required | 🟢 active exact one-file claim | `src/db/seed.ts` | Remove synthetic freshness-to-trust ladder | 2026-07-18 | Current generated observations are uniformly `provenance: "synthetic"` while the offer projection still maps freshness to `high`/`medium`/`low`. The accepted correction is exactly `trustLevel: "none"` in the generated `offersCurrent` row. Preserve freshness, catalog, observations, prices, units, availability, timestamps, and counts. No seed/DB/schema/migration/read-side/UI operation; one path-scoped commit followed by independent static refutation. |
