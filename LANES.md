...
| Maps Engineering Lead | Active place-marker owner; named queued self-location owner | Finish the exact two-path contextual place-marker slice, then begin the queued self-location/avatar vertical immediately after both Maps and Contribution release its intended paths; no queued edit authority exists yet |
...
| Identity & Email Delivery — branded Neon Auth OTP | **Closed / released with VERIFIED monochrome production evidence** | Implementation `64d3519`, type correction `8583495`, and Founder-requested monochrome visual correction `d549d42` are pushed on `origin/main`. The correction touched only `src/lib/auth-email.ts` and `scripts/auth-email-contracts.test.ts`; focused contract PASS and independent default-to-refuted VERIFIED with no P1/P2/P3. Production deployment `dpl_D7NXRhdh4eiUniiVEiEtdxUsyCnH` is Ready and aliased to `www.wetindey.live`; a fresh production OTP POST returned `200`, and Gmail confirmed the exact subject from `WetinDey auth@wetindey.live`. All six implementation paths remain released; process-local caches, cross-instance Message-ID limits, provider Beta status, SPF/DKIM/DMARC monitoring, and email-client rendering remain operational follow-up |
...
sign-in. This closes and releases all six implementation paths while preserving the
...
and authorizes no edit while Contribution owns `src/app/page.tsx` or the contextual
place-marker lane owns the two current Maps paths.

Implementation begins immediately after both current owners release. The exact intended
post-release vertical slice is:
...
| **Self-location avatar marker — signed-in and guest** | 🟡 named queued vertical; responsibility assigned, paths unclaimed until both current owners release | persistent Maps Engineering Lead implementation owner; Iconography & Visual Systems Lead visual reviewer; independent Privacy refuter | Intended post-release paths: `src/app/page.tsx`; `src/design-system/components/MapboxCanvas.tsx`; `src/integrations/maps/MapboxAdapter.ts`; conditional new `src/design-system/components/SelfLocationMarker.tsx` only if implementation proves it useful; new `scripts/map-self-location-contract.test.ts` | Signed-in self uses uploaded avatar or local initials fallback; guest self uses a bundled/local “me” fallback. Precise GPS gets bounded precise treatment; approximate/area uses an identity orb plus diffuse accuracy halo, never blur instead of identity. Require truthful screen-reader labels, light/dark/grayscale, non-obstruction, no external avatar generator, peer publication, Presence rollout, false precision, DB/schema/migration. No edit authority until Contribution and the active Maps lane both release |
...
|---|---|---|---|---|
| **G0 governance reconciliation** | event-driven employee upkeep, docs by default | Quality & Release Controller | Reconcile checkpoint evidence, ownership, gates, and employee handoffs only when explicitly assigned | Standing ownership remains `LANES.md` and `docs/architecture/RELEASE-CONTROLLER.md`; the completed branded Neon Auth OTP exception and its six implementation paths are released, with no continuing app/package/schema/migration authority |
...
| **Identity & Email Delivery — branded Neon Auth OTP** | completed and released at `64d3519` + `8583495`; Founder-requested monochrome correction `d549d42` production VERIFIED | current controller implementation; independent default-to-refuted reviewers | Released paths: `src/app/api/webhooks/neon-auth/route.ts`; `src/lib/auth-email.ts`; `scripts/auth-email-contracts.test.ts`; `docs/operations/AUTH-EMAIL-DELIVERY.md`; `package.json`; `package-lock.json`; read-only `public/logo.png` remains unclaimed | Original independent VERIFIED with no P1/P2/P3; focused contract, exact-path lint, and build passed. Hostinger, Vercel, Neon webhook, fail-closed route, and real sign-in evidence remain recorded above. After the Founder rejected green, `d549d42` changed only the renderer and focused contract to true black/white neutral, text wordmark, fluid mobile-safe layout, stronger small-text contrast, and no remote imagery/colour leakage; focused PASS and independent VERIFIED no P1/P2/P3. Deployment `dpl_D7NXRhdh4eiUniiVEiEtdxUsyCnH` is Ready/aliased; fresh OTP POST `200` and Gmail confirmed a new unread message from `WetinDey auth@wetindey.live` with exact subject. Process-local caches, cross-instance Message-ID limits, provider Beta, SPF/DKIM/DMARC monitoring, and email-client rendering remain operational follow-up, not path claims |
...
| `src/app/page.tsx` | ~1300 | **Contribution containment and integrity** — `495750a` runtime REFUTED on focus escape; retained pending the current Motion correction and independent runtime verdict; H38 remains pathless and blocked. The named self-location/avatar vertical is a queued successor only and has no claim until release |
...
| `src/app/api/webhooks/neon-auth/route.ts`, `src/lib/auth-email.ts`, `scripts/auth-email-contracts.test.ts`, `docs/operations/AUTH-EMAIL-DELIVERY.md` | — | **released** after branded Neon Auth OTP implementation `64d3519`, type correction `8583495`, and Founder-requested monochrome correction `d549d42`; both review rounds VERIFIED with no P1/P2/P3, focused gates passed, latest Production deployment is Ready/aliased, and fresh production POST plus Gmail evidence passed |
...
| `src/design-system/components/MapboxCanvas.tsx`, `src/integrations/maps/MapboxAdapter.ts` | Maps Engineering Lead | **Contextual place-marker orbs and selected glow** — active exact two-path claim; preserve H37 semantics and all exclusions recorded above. The self-location/avatar vertical is the named immediate successor after release, not a concurrent claim |
...
| Maps Engineering | Place/market symbols, selected marker state, map visual language, and map-specific runtime refutation | Persistent Lead active on exact `MapboxCanvas.tsx` + `MapboxAdapter.ts` contextual-orb/selected-glow slice; named self-location/avatar vertical queued immediately after Maps and Contribution release; peer work remains separate |
...
| Seller Platform / Identity & Access / Email Delivery | Seller onboarding, scoped RBAC, verification, consented contact, seller operations, and auth-message delivery | Seller architecture remains unassigned; branded Neon OTP via Hostinger is completed/released at `64d3519` + `8583495`, with Founder-requested monochrome correction `d549d42` independently VERIFIED and live at Ready deployment `dpl_D7NXRhdh4eiUniiVEiEtdxUsyCnH`; operational deliverability monitoring remains |
...
3. **Map Symbols, self location, and Nearby Presence**: Maps Engineering actively owns
   only the two place-marker rendering paths for contextual orbs and selected glow. Its
   named self-location/avatar vertical starts immediately after that lane and Contribution
   release, but claims nothing yet. Presence / Safety owns its separate backend/privacy
   lane; peer UI and camera changes remain separate. Never use simulated centres to unlock
   real peers.
