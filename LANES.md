...
| Security & Privacy Engineering Lead | Active collector lane only; nonce source paths released | Preserve the report-only collector claim. Production nonce propagation is now a pathless upstream Next/Vercel adapter gate; enforcement remains prohibited |
...
| Security & Privacy | CSP reporting, privacy disclosures, account deletion, presence safety, redaction, retention, and independent refutation | Collector source remains active/report-only. Production nonce P1 is REFUTED/pathless upstream with three source paths released; deletion provider proof remains REFUTED/pathless; enforcement prohibited |
...
6. **CSP Reporting and production nonce P1**: Collector redaction/retention remains
   active and report-only. The nonce source experiment is closed and all three paths are
   released; the P1 remains pathless at the upstream Next/Vercel adapter boundary after
   repeated Preview 6/29 nonce evidence. Enforcement is prohibited. Reopen only on
   materially new adapter/version evidence, not another identical experiment.
