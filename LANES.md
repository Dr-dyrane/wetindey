...
| **Contribution containment and integrity** | queued read-only/pathless pending exact containment plan | Contribution Integrity & Moderation Engineer `019f75a3-f50d-7180-8e92-0a7aabd8a98c` | First design how to fail closed unsafe public contribution behavior, then propose transactional observation/projection writes, server idempotency, atomic conflict handling, rate policy, and non-destructive contradictory-report state | Owns no paths. `src/app/page.tsx` is withheld by the active HI 44px forward-correction lane until its independent visible-Safari refutation and release. Presence owns forward migration `0012`; contribution migration is `0013` or later. Future implementation requires exact claims, threat model, disposable integration proof, independent refutation, and no automatic moderation fiction |
...
| **Human Interface: category + Aboki FX flattening** | `4b572e0` remains REFUTED pending direct pixels; component paths have no current static defect or write claim; `page.tsx` transferred forward | HI Quality employee refuter / Quality & Release Controller evidence custody | Required pixels were unavailable in the first refutation; static category/Aboki changes had no scoped defect, while the confirmed 32x32 Add/Profile targets required the forward correction below | `src/app/_components/CategorySelectorSheet.tsx` and `src/app/_components/ExchangePanel.tsx` remain unclaimed but pending direct-pixel evidence. HI Quality is assigned visible-Safari refutation of `4b572e0` and `d82b87e`. `src/app/page.tsx` remains claimed until verdict |
| **HI Add/Profile 44px hit regions** | committed at `d82b87e`; unreleased pending independent visible-Safari refutation | Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d` | Make Add/Profile interactive hit regions at least 44x44 while preserving the current visible icon/avatar scale | Commit `d82b87e11faa643840c322e6b1d9fbafd2ee356d` changed exactly `src/app/page.tsx`. HI Quality must refute that exact commit in visible Safari. Do not release the path before that PASS; no category component, Exchange component, data, action, schema, migration, presence, or contribution edit |
...
| **Catalog seed source-only follow-up** | queued read-only/pathless pending an exclusive future lane | Catalog Stewardship Engineer `019f75a3-f38d-7893-9b82-2d6871a2563c` | Inspect and design source-only catalog/seed correction and evidence | Owns no paths now. A future exclusive lane may claim exactly `src/db/seed.ts` and `src/db/seedContent.ts`; no database access or reseed. Preserve the completed `91243c4` evidence boundary, obtain independent refutation, and report that source correctness is not database state |
| **Recurring Lagos/Nigeria food-news ingestion design** | queued lane design only; owns no paths | Sol provenance lead `019f75a8-7eb2-7be2-9bbc-781e318bda0d` active read-only; Terra fixture engineer `019f75a8-7d3b-7482-b7e2-ce8ed35b9491` and Luna source refuter `019f75a8-8062-7330-93ee-1fb0d8d4fbc5` queued/pathless | Propose exact new paths for attributable retrieval/review artifacts, append-only candidate staging, and a separate provenance-aware development-only current-food-news fixture; Sol implements after exact paths are claimed | NBS review-only `active + fetch_and_stage` is standing-approved and must not be gated as approval pending. No path may be claimed before Sol returns the exact proposal. Existing seed/demo bytes are immutable; `src/db/seed.ts` is forbidden for recurring ingestion; source-backed evidence never uses `Sample`; origin copy names source + geography + period; no inferred availability or market/place overstatement; shared/production `0010`, live publication/promotion, push, and deployment remain blocked separately |
...
| **Presence Platform implementation design** | active read-only assignment; owns no paths | Presence Platform Engineer | Produce the bounded implementation design after accepted ADR-016 without editing product, schema, or migration paths | Sequence remains forward `0012` schema/migration, server boundary, consent/lifecycle, snapshot lifecycle, map DTO/UI, independent refutation, and separately authorized target migration/pilot. Exact target inventory/compatibility, counsel, safety responder, rate budgets, two-account Festac allowlist, default-off app flag, database kill switch, retention, and rollout authorization remain unresolved gates |
...
| `src/app/_components/CategorySelectorSheet.tsx` | — | **unclaimed; direct-pixel evidence pending** for `4b572e0`; HI Quality found no current static defect |
| `src/app/_components/ExchangePanel.tsx` | — | **unclaimed; direct-pixel evidence pending** for `4b572e0`; HI Quality found no current static defect |
| `src/app/page.tsx` | ~1300 | **HI Add/Profile 44px hit regions** — commit `d82b87e`; Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; unreleased pending independent visible-Safari refutation |
...
| `docs/adr/016-nearby-user-presence.md`, `WETINDEY_BIBLE.md` | — | **released** after path-scoped `c96086007e6f379c1b686b8203deef2c7c5559c2` and independent PASS; ADR-016 acceptance is implementation architecture only |
...
| **Human Interface: category + Aboki FX flattening** | Human Interface employee; HI Quality refuter; Quality & Release Controller evidence custody | 🔴 `4b572e0` REFUTED pending visible-Safari pixels; component paths unclaimed; `page.tsx` transferred | — | Founder-directed Human Interface correction | 2026-07-18 | Required pixels were unavailable through the first refuter connector. Static category/Aboki changes otherwise had no scoped defect. `CategorySelectorSheet.tsx` and `ExchangePanel.tsx` have no current write claim but remain pending direct-pixel evidence. HI Quality is assigned visible-Safari refutation of the remaining `4b572e0` pixels and forward commit `d82b87e`. No push or behavioral completion claim follows. |
| **HI Add/Profile 44px hit regions** | Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; HI Quality independent refuter | 🔵 committed at `d82b87e`; visible-Safari refutation assigned | `src/app/page.tsx` | Human Interface forward correction | 2026-07-18 | Path-scoped commit `d82b87e11faa643840c322e6b1d9fbafd2ee356d` changed only `src/app/page.tsx`, expanding Add/Profile hit regions to 44x44 while keeping the visible circle/avatar 32x32. HI Quality is assigned independent visible-Safari refutation of this exact commit and the remaining `4b572e0` pixels. The path remains claimed and `NO PUSH` remains until verdict and explicit release handoff. |
...
| **Catalog seed source-only follow-up** | Catalog Stewardship Engineer `019f75a3-f38d-7893-9b82-2d6871a2563c` | 🟡 queued read-only | — | Catalog stewardship | 2026-07-18 | Owns no paths and performs no database access or reseed. A later exclusive implementation lane may claim only `src/db/seed.ts` and `src/db/seedContent.ts` after coordination and must distinguish source evidence from database state. |
| **Recurring food-news ingestion path design** | Sol provenance lead `019f75a8-7eb2-7be2-9bbc-781e318bda0d` | 🟡 active read-only | — | Founder-authorized attributable food-news retrieval/review | 2026-07-18 | Propose exact new paths only; after those paths are claimed, proceed to implementation. Do not claim or create fixture paths yet. NBS review-only `active + fetch_and_stage` is standing-approved and must not be requested again. Source-backed evidence never uses `Sample`; required copy names explicit source + geography + period, e.g. `Official data · NBS · Lagos State · May 2026`. Existing seed/demo bytes remain immutable, and `src/db/seed.ts` is prohibited for recurring ingestion. |
| **Development food-news fixture implementation** | Terra fixture engineer `019f75a8-7d3b-7482-b7e2-ce8ed35b9491` | 🟡 queued/pathless | — | Provenance-aware development fixture | 2026-07-18 | Wait for Sol's exact-path proposal and explicit lane transfer. No current path ownership, seed/demo mutation, database access, live promotion, push, or deployment. |
| **Food-news source refutation** | Luna source refuter `019f75a8-8062-7330-93ee-1fb0d8d4fbc5` | 🟡 queued/pathless | — | Source/evidence refutation | 2026-07-18 | Wait for exact candidate paths and evidence. Refute attribution, timestamps, hashes, external raw pointers, deterministic review artifacts, append-only staging, source terms, inferred availability, and market/place claims. Owns no implementation path. |
| **Contribution containment plan** | Contribution Integrity & Moderation Engineer `019f75a3-f50d-7180-8e92-0a7aabd8a98c` | 🟡 queued read-only | — | Contribution integrity / moderation | 2026-07-18 | Owns no paths pending an exact containment plan. `src/app/page.tsx` remains owned by the HI Add/Profile 44px lane until its independent visible-Safari PASS and explicit release handoff. Any contribution migration is `0013` or later, after presence-owned forward `0012`. |
...
| **Presence Platform implementation design** | Presence Platform Engineer | 🟡 active read-only | — | ADR-016 implementation design | 2026-07-18 | Owns no repository paths. The assignment may inspect and design only. Any engineering handoff must name exact paths and preserve the order: forward `0012` schema/migration, server boundary, consent/lifecycle, snapshot lifecycle, map DTO/UI, independent refutation, then separately authorized target migration/pilot. |
...
| **P0 search schema + provenance repair** | controller / Sol with fresh executable refuter | 🔵 code committed and independent static refutation complete; read-only disposable-SQL evidence gate pending | — | Food Truth & Pilot Operations / Stage 0 | 2026-07-18 | Commit `c6f304b` completed the schema/provenance correction and fallback-context hardening. All write claims, including `src/app/actions.ts`, are released. Remaining disposable SQL execution is evidence collection only and cannot lock or modify a code path. No shared DB, schema/migration, ranking, category, push, or deployment. |
