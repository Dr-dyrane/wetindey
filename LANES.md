...
| **Human Interface: category + Aboki FX flattening** | implementation committed at `4b572e0`; paths remain claimed pending independent refutation and release handoff | Human Interface employee; Quality & Release Controller holds release evidence only | Flattened the category selector and simplified Aboki FX to an amount-first hierarchy without duplicated explanatory copy | Owns exactly `src/app/_components/CategorySelectorSheet.tsx`, `src/app/_components/ExchangePanel.tsx`, and `src/app/page.tsx` until direct Safari light/dark and compact/desktop refutation passes; no map/data/actions/schema/migration/reviews/presence changes |
...
| `src/app/_components/CategorySelectorSheet.tsx` | — | **Human Interface: category + Aboki FX flattening** — CEO controller / next HI worker |
| `src/app/_components/ExchangePanel.tsx` | — | **Human Interface: category + Aboki FX flattening** — CEO controller / next HI worker |
| `src/app/page.tsx` | ~1300 | **Human Interface: category + Aboki FX flattening** — CEO controller / next HI worker; only removal/relocation of the long persistent Sample strip if needed. |
...
| **Human Interface: category + Aboki FX flattening** | Human Interface employee; Quality & Release Controller evidence custody | 🔵 committed at `4b572e0`; independent refutation pending | `src/app/_components/CategorySelectorSheet.tsx`; `src/app/_components/ExchangePanel.tsx`; `src/app/page.tsx` | Founder-directed Human Interface correction | 2026-07-18 | Implementation is committed but the lane is not released. Require independent direct Safari light/dark and compact/desktop visual refutation tied to `4b572e0`, including 44px targets, disabled semantics, honest Sample disclosure, readable surface contrast, and no text/icon collisions. No push or behavior claim follows from the commit alone. |
...
| **Contribution containment plan** | Contribution Integrity & Moderation Engineer `019f75a3-f50d-7180-8e92-0a7aabd8a98c` | 🟡 queued read-only | — | Contribution integrity / moderation | 2026-07-18 | Owns no paths pending an exact containment plan. `src/app/page.tsx` remains owned by the HI lane until `4b572e0` receives independent refutation and an explicit release handoff. Any contribution migration is `0013` or later, after presence-owned forward `0012`. |
