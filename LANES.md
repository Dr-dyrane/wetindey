...
#### Docs-truth wave 4: product-doc path refresh - ACTIVE

Owner: Private Contractor, Maps Delivery `c9c17443-ef5e-4a7b-9b6e-c8f5381da30c`; the last routed docs-truth tier. COMMUNITY-TRUST-AND-CONTRIBUTION-LOOP.md cites src/app/actions.ts and flat component files that became directories; the fix converts its stale citations to file-and-symbol form with every target verified at HEAD, prose meaning unchanged (it is a dated audit; corrections mark themselves as path refreshes, not re-audits). WETINDEY_BIBLE.md's normative-reading hazard ("src/app/actions.ts is the contract", the trust-batch one-consumer claim) gets the same bracketed-amendment treatment wave 2 used, pointing at the SERVICE-ARCHITECTURE amendment as the current truth. Exact paths: `docs/product/COMMUNITY-TRUST-AND-CONTRIBUTION-LOOP.md`; `docs/WETINDEY_BIBLE.md`. Completion: every new cite resolves, original prose preserved where it is a dated record, no em dashes, gates green, independent default-to-REFUTED refutation, path-scoped commit, release.
