...
#### i18n copy riders: remaining hoist residuals - ACTIVE

Owner: Private Contractor, Maps Delivery `c9c17443-ef5e-4a7b-9b6e-c8f5381da30c`, self-claimed; the rider list recorded by the released i18n hoist lane. Twelve new keys, en verbatim (curly apostrophe in the outside-areas title preserved byte-for-byte), pidgin and yoruba UNTRANSLATED: Visit shop / Visit market and the prices count suffix on the home surfaces, the recenter control aria-label, and the eight strings of the location sheet's local copy module, which is deleted with both consumers repointed at the dictionary. The Close aria-label reuses the existing vouched `close` key. Ruled OUT with reason: useHomePage's document.title effect duplicates layout.tsx's canonical brand metadata; localizing the tab title while server metadata stays English is a brand and SEO divergence, not a copy hoist, so it stays as is.

Exact paths: `src/core/i18n/strings.ts` (hotspot, short-lived); `src/app/_components/home-page/views/HomePlaceDetailView.tsx`; `src/app/_components/home-page/views/HomeSheetResultsView.tsx`; `src/design-system/components/MapboxCanvas.tsx` (aria-label only); `src/app/_components/location-sheet/views/LocationSheetView.tsx`; `src/app/_components/location-sheet/hooks/useLocationSheet.ts`; delete `src/app/_components/location-sheet/copy/copy.ts`; `scripts/i18n/copy-hoist-contract.test.ts` (extend sweeps).

Completion: rendered en copy byte-identical, contract green with the new sweeps, gates green, independent default-to-REFUTED refutation, path-scoped commit under the `0bbdb11` class, span-checked release.
