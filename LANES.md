...
#### Read-hygiene riders: robots rule, sitemap N+1, trust freshness window - ACTIVE

Owner: Private Contractor, Maps Delivery `c9c17443-ef5e-4a7b-9b6e-c8f5381da30c`, the remaining bounded items from discovery sweep two. Three fixes: robots.ts disallows /zzboom, a route deleted from the tree (drop the dead rule and its stale citation); sitemap.ts awaits hasObservedEvidence serially per catalog row (batch with a bounded-concurrency map, same output order and bytes); the trust read in src/lib/food-trust.ts fetches unbounded observation history and discards stale rows in JS (add an observed_at window predicate consistent with FRESHNESS_POLICY, verifying assessTrust semantics are unchanged for every row the window admits; if any assessTrust branch reads beyond the freshness horizon, keep that class unbounded and record why). Exact paths: `src/app/robots.ts`; `src/app/sitemap.ts`; `src/lib/food-trust.ts`; extend `scripts/actions/server-action-hardening-contract.test.ts`. Completion: contract green in the static stage, gates green, sitemap output proven byte-identical, independent default-to-REFUTED refutation, path-scoped commit, release.
