...
| Private Contractor, Maps Delivery `c9c17443-ef5e-4a7b-9b6e-c8f5381da30c` | Fifteen lanes complete / released; seat idle on the check-in loop | ADR-031 lane 3 map surface RELEASED at `7707515` (evidence WD-MAPS-AGRISURFACE-20260722-5), dormant behind the default-off flag; map machinery engine-proven on Chromium (Metal and SwiftShader) and WebKit; residual Maps scope only Safari shell and iOS device; Presence serialization when routed |
...
#### i18n hoist: hardcoded user-facing English into strings.ts - ACTIVE

Owner: Private Contractor, Maps Delivery `c9c17443-ef5e-4a7b-9b6e-c8f5381da30c`, self-claimed under the Founder's standing continuous-work directive; the residual recorded by the released design-system a11y lane. Moves every hardcoded user-facing string at the scouted sites into src/core/i18n/strings.ts: en verbatim from the code (including the "We no fit ..." register, which is this app's English per the profile.save_error precedent); pidgin and yoruba answered UNTRANSLATED, never invented. Revives the six dormant geo.err_*_title keys; three take en alignment edits (cannot, could not, did not) to match the live locationStore copy verbatim. Runtime copy is unchanged everywhere: this lane changes where copy lives, not what it says. locationStore's DeviceLocationProblem drops title/message for titleKey/messageKey (type-only StringKey import, so the node contract tests gain no runtime i18n dependency); its three consumers translate via useT(). EXCLUDED: report-price-sheet EvidenceMediaField.tsx (evidence-media vertical owns it); riders like share copy and aria-labels stay out and are listed at release.

Exact paths: src/core/i18n/strings.ts (shared hotspot, serialized, short-lived claim); src/core/i18n/index.ts (one line, export type StringKey); src/core/state/locationStore.ts; src/design-system/components/BottomSheet.tsx; src/design-system/components/SheetPicker.tsx; src/design-system/components/AsyncList.tsx; src/design-system/components/MapboxCanvas.tsx (MapRecenterControl onError only; noted: queued Presence integration path, not an active claim); src/app/_components/home-page/hooks/useHomePage.ts; src/app/_components/home-page/views/HomeSheetResultsView.tsx; src/app/_components/home-page/views/HomePlaceDetailView.tsx; src/app/_components/home-page/views/HomePageView.tsx (drop the results view's t prop); src/app/_components/location-sheet/hooks/useLocationSheet.ts; src/app/_components/get-it-sheet/hooks/useGetItSheet.ts; new scripts/i18n/copy-hoist-contract.test.ts.

Completion: rendered en copy byte-identical (locale drive comparing before/after visible strings on the touched surfaces); tsc silent (the LocaleTable completeness gate); location contract 20 of 20; independent default-to-REFUTED refutation; path-scoped commit pushed under the `0bbdb11` class; span-checked release.
