...
| **Contribution containment and integrity** | `495750a` runtime REFUTED on focus escape; shared `ModalSheet` correction `f7cddba` + `9fe9d4e` independently NOT REFUTED; Safari direct partial runtime PASS | Contribution Integrity & Moderation Engineer; Newton runtime refuter; controller Safari evidence | Shared focus lifecycle now excludes disabled controls and disabled fieldsets, with panel fallback | Existing Safari localhost tab opened Report Food Price with all report controls disabled and focus landed on the enabled in-sheet Close button. ConfirmVisit, Tab/Shift+Tab, Escape/restoration, and storage/network/visit residuals remain unproven. Five contribution paths remain retained; H38, H31, and market-detail work stay gated. No DB/push/deploy |
...
| **Controlled Semantic Iconography implementation** | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac`; Schrodinger independent refuter; Aristotle runtime verifier | ✅ committed at `69737de`; static NOT REFUTED; paths released | `src/app/globals.css`; `tailwind.config.ts`; `src/design-system/components/IconOrb.tsx`; `src/design-system/components/ListRow.tsx`; `src/design-system/components/ItemCard.tsx`; `src/app/_components/CategorySelectorSheet.tsx`; `src/app/_components/ProfileSheet.tsx`; `src/app/_components/GetItSheet.tsx`; `src/app/_components/AboutSheet.tsx`; `src/app/_components/LocationSheet.tsx`; `src/app/_components/ReportProblemSheet.tsx`; `scripts/iconography-contracts.test.ts` | ADR-018 implementation | 2026-07-18 | Static refutation found no P1/P2 and the focused contract check passed. Runtime verifier was blocked before tab discovery by the existing browser bootstrap error `Cannot redefine property: process`; no visual/accessibility PASS or defect is claimed. Contrast recalculation and runtime proof remain open. |
...
| H27 | **queued trust/seed follow-up; no current path ownership** | `src/db/seed.ts` still stamps a hardcoded freshness-to-trust ladder, bypassing derived read-side trust. Current HEAD also retains stale deleted-`getFoodItemCandidates` comments in `src/lib/trust.ts` and `src/app/actions.ts`. Connected seeded rows and any destructive refresh remain separate DB gates. | Current HEAD: `src/db/seed.ts` `trustLevel` assignment and greppable stale comments. | **OPEN; no path claim, no reseed/DB authority.** |
...
| H37 | **superseded/resolved; no current path ownership** | Current HEAD carries `placeType` through the offer query and default-place projection into page marker candidates and `MapboxCanvas` marker creation. The former claim that the map never receives WetinDey place types is stale. | Current HEAD: `src/app/actions.ts`, `src/app/page.tsx`, and `src/design-system/components/MapboxCanvas.tsx`. | **CLOSED; former map-data paths released. Separate market-detail offer-surface handoff remains pathless.** |
