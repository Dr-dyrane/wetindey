...
**Pushing — this rule was wrong and is corrected.**

It read *"never push, ever, unsupervised"*. That was too absolute, and it broke on contact:
the owner told another session **"handle git, check for good checkpoints and push"**, which
the rule then forbade. That session had to knowingly override this file to obey the owner
(H11). **A rule that forces a session to choose between the owner and the contract is a bug
in the rule.** The owner outranks this file — always. So:

- **An explicit owner instruction to push authorises pushing.** It does not expire when they
  step away; it expires when the work stops resembling what they authorised.
- **Never push the irreversible, even when told to push.** Destructive migrations, account
  or data deletion, anything that cannot be undone by a revert. Commit it locally, say it is
  ready, and let a human say go. "Push the checkpoints" is not "push the deletion".
- **Never push what you have not verified.** There are zero tests here; a push is the last
  moment anyone could have caught it. The rule exists because of a real event — a session
  pushed schema-dependent code before its migration and **broke every write path in
  production**. That is the failure this guards, and it was a *specific* mistake, not the act
  of pushing.
- **No instruction, no push.** Silence is not authorisation.

**Do NOT, ever, unsupervised:**
- **Deploy, or send anything else off this machine** that is not a push the owner asked for.
