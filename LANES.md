...
| **H6 nonce-CSP implementation** | exact conflict-free three-path lane assigned; read-only readiness only in this checkpoint | owner `/root/h6_nonce_implementation`; independent refuter `019f7692-d254-7e43-a687-154876274927` after commit | `src/middleware.ts` (new); `src/app/layout.tsx`; `vercel.json` | Accept only one canonical per-request CSP; cloned request headers with raw `x-nonce` plus exact CSP; byte-identical response CSP; nonce all three raw layout scripts and Mapbox script; remove only static Vercel CSP while preserving other headers; private/no-store and environment contracts; no product/layout redesign. No code started in this reporting checkpoint |
...
| `src/middleware.ts`, `src/app/layout.tsx`, `vercel.json` | — | **H6 nonce-CSP implementation** — prospective owner `/root/h6_nonce_implementation`; exact three-path conflict-free lane, read-only readiness only; independent refuter `019f7692-d254-7e43-a687-154876274927` required after commit |
...
| **H6 nonce-CSP implementation** | prospective owner `/root/h6_nonce_implementation`; independent refuter `019f7692-d254-7e43-a687-154876274927` | 🟢 exact conflict-free three-path assignment; read-only readiness only | `src/middleware.ts`; `src/app/layout.tsx`; `vercel.json` | ADR-020 implementation | 2026-07-18 | Acceptance: one canonical per-request CSP; cloned request headers carry raw `x-nonce` plus exact CSP; identical response CSP; nonce all three raw layout scripts and Mapbox script; remove static Vercel CSP in the same change while preserving all other headers; private/no-store and environment contracts; no product/layout redesign or second enforcement boundary. No code began in this reporting checkpoint. |
