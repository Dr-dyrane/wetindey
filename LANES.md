...
#### Server-action read hardening - ACTIVE

Owner: Private Contractor, Maps Delivery `c9c17443-ef5e-4a7b-9b6e-c8f5381da30c`, from discovery sweep two. Five verified gaps in public server actions: getOfferTrustBatch/getOfferTrust exported with zero validation, unbounded key arrays, no LIMIT and no freshness cutoff and zero callers (become internal or parsed+bounded); getReviewsForEntity/getReviewAggregate cast unparsed input to uuid (driver 500 on garbage) with no LIMIT; getPopularItems' legacy no-coords signature interpolates an empty SQL fragment into the cohort query (reachable 500); evidence-media reads skip the uuid parse discipline; searchItems' category is uncapped. All fixes are parse-at-the-boundary and bound-the-read, no behavior change for well-formed callers. Exact paths: `src/app/_actions/food-actions.ts`; `src/app/_actions/review-actions.ts`; `src/app/_actions/evidence-media-actions.ts`; `src/app/_actions/actions.ts` (only if exports change); `src/lib/food-trust.ts`; `src/lib/validation.ts`; new `scripts/actions/server-action-hardening-contract.test.ts`. Completion: contract green, gates green, independent default-to-REFUTED refutation, path-scoped commit under the `0bbdb11` class, release.

#### Contract-test rot repair - ACTIVE

Owner: same seat. Four unenforced contract tests are broken against HEAD: motion-contracts pins a single-line focus call that gained preventScroll; modularization-structure requires a copy/ dir the i18n hoist legitimately deleted; report-vocab-lazy-load's slice regex assumed an empty dep array that gained t (this seat's own earlier lane, self-caught by sweep); contribution-moderation-runtime imports server-only and cannot run under the tsx harness. Exact paths: `scripts/motion-contracts.test.ts`; `scripts/modularization-structure-contract.test.ts`; `scripts/contributions/report-vocab-lazy-load-contract.test.ts`; `scripts/contributions/contribution-moderation-runtime-contract.test.ts` (harness compatibility only, no assertion weakening). Completion: all four green under npx tsx with assertions equal or stronger, refuter confirmation, path-scoped commit, release.
