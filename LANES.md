...
| H18 | **whoever owns the design system's focus ring** | **A `Button` in `:focus-visible` gets the UA's ring, not ours — and I could not find out why.** Removing `focus-visible:outline-none` from `Button.tsx` (P0-1) restored *a* ring, but not the designed one. Measured under a real Tab on `Submit Report`: `white solid 1.5px, offset 0` — Chrome's default. A bare `<button>` appended to `document.body` in the same page gets the real thing: `rgb(0,122,255) solid 2px, offset 2px`, straight from `--color-focus-ring`. **So `globals.css`'s `:focus-visible` rule works — it just is not winning on this component.** Nothing in `Button.tsx` or `ModalSheet.tsx` sets `outline`; no utility sets it; the class is gone from the element (verified in the DOM). Not a transition — it is stable at 900ms. Candidates I did not exhaust: `@layer` ordering between Tailwind's layers and the `base` rule, or a UA rule I am misreading. **A blue 2px ring at 2px offset is the design; a white 1.5px hairline is not** — and on `bg-accent` it is nearly invisible. | `plainProbe` vs `Submit Report`, same page, same moment. | **Open.** |
