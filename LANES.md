...
| Iconography & Visual Systems Lead | Foundation source complete; visual evidence blocked | Commit `284a685` has focused/static PASS and no code finding; resume light/dark native-pixel refutation only when an uncontested existing localhost window is available |
...
### Blocked lane: Item Offer Decision & Get-It Handoff Visual Adoption

The four item-offer adoption paths below are released and must not be edited until the broader
...
### Evidence-gated lane: Semantic iconography foundation
...
|---|---|---|---|
| **Semantic iconography foundation** | 🟡 source complete at `284a685`; focused contract and independent static PASS; runtime pixels blocked by externally contested browser | Iconography & Visual Systems Lead implementation owner; independent visual/accessibility refuter | Exact five-path commit provides opaque accessible light/dark fill/ink pairs, exact orb/glyph mappings, caller-owned-color prohibition, forced-colors inheritance, and decorative semantics. Repeated browser bridges failed or the active meeting switched the shared tab before native capture; no product P1/P2/P3 was established | Paths released from editing into evidence-gated state. Remaining adoption and map-symbol work must not claim visual completion until an uncontested existing localhost window yields native light/dark evidence |
| **Item Offer Decision & Get-It Handoff Visual Adoption** | 🟡 useful adjacent slice, unclaimed; separate from the Founder’s Market Details complaint | Iconography & Visual Systems Lead; independent visual/accessibility refuter | `src/app/_components/ItemDetailSheet.tsx`; `src/design-system/components/ListRow.tsx`; `src/design-system/components/SheetPicker.tsx`; `src/app/_components/GetItSheet.tsx` | Improve the item → variant/unit → ranked places → directions/share/contact journey using the accepted visual system. This slice does not own or repair the place/market detail shown in the Founder’s screenshot |
...
| **H6 CSP report collector and retention** | **ACTIVE exact source claim**; private storage capability proved; deployment still gated | Security & Privacy Engineering Lead; fresh independent privacy/retention refuter | `src/app/reports/security/csp/route.ts`; `src/app/reports/security/csp/cleanup/route.ts`; `src/lib/security/csp-report.ts`; `src/lib/security/csp-report-store.ts`; `.env.example`; `vercel.json` (Cron addition only); `scripts/csp-report-contracts.test.ts` | Implement the approved first-party public POST collector, pre-persistence sanitization, private Blob namespaces, 14/30/90-day retention, authenticated cleanup, bounded payloads, fail-open storage, and exact focused contracts. Keep the existing static CSP byte-for-byte. No push/deploy/WAF/provider mutation |
| **H6 nonce CSP report-only** | **ACTIVE disjoint source claim**; enforcement remains prohibited | dedicated Security implementation worker; fresh independent Next/CSP refuter | `src/middleware.ts`; `src/app/layout.tsx`; `src/lib/security/csp-policy.ts`; `scripts/csp-policy-contracts.test.ts` | Per-request nonce on cloned Next request headers plus byte-identical browser `Content-Security-Policy-Report-Only`; nonce all live inline/Mapbox scripts; dynamic private/no-store HTML; exclude collector/cleanup routes; preserve the existing Vercel enforcing CSP. No static-CSP removal, enforcement, push, or deploy |
...
| `src/middleware.ts`, `src/app/layout.tsx`, `src/lib/security/csp-policy.ts`, `scripts/csp-policy-contracts.test.ts` | dedicated Security implementation worker | **H6 nonce report-only** — active exact disjoint claim; current static Vercel CSP remains the sole enforcing policy |
...
| `src/app/globals.css`, `tailwind.config.ts`, `src/design-system/components/IconOrb.tsx`, `src/design-system/icons/SolidIcon.tsx`, `scripts/iconography-contracts.test.ts` | released; Iconography & Visual Systems Lead retains evidence ownership | **Semantic iconography foundation** — source complete at `284a685`; visual evidence gate remains open due externally contested browser, not a proven code defect |
| `src/app/_components/ItemDetailSheet.tsx`, `src/design-system/components/ListRow.tsx`, `src/design-system/components/SheetPicker.tsx`, `src/app/_components/GetItSheet.tsx` | unclaimed; future Iconography & Visual Systems adoption | **Item Offer Decision & Get-It Handoff Visual Adoption** — adjacent item-centric journey, not the place/market detail complaint |
...
| **H6 nonce CSP report-only** | dedicated Security implementation worker; fresh Next/CSP refuter | 🟢 active disjoint source claim | middleware, layout, policy module, focused contract | ADR-020 report-only delivery | 2026-07-18 | Implement cloned-request nonce propagation and byte-identical response report-only policy while preserving the existing static enforcing CSP. Enforcement, static-CSP removal, public-avatar origin admission, push, and deployment remain separately prohibited. |
...
1. **Market Details — Place Offer Surface Redesign**: Human Interface with Iconography support, after Contribution releases `page.tsx` and `actions.ts`;
