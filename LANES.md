...
| Human Interface Design Engineer | Active one-file Market Details compact CTA correction; one evidence path retained | Own only `src/app/page.tsx` for the sticky Get-It flow-footprint/offset correction. `PlaceOfferRow.tsx` remains retained for combined runtime evidence with no widening |
...
| Iconography & Visual Systems Lead | Active: two retained evidence paths plus one disjoint Report Price correction | HI runtime matrix passed and released Settings, Currency Picker, and Exchange Panel. Retain only `SheetPicker.tsx` as runtime-unverified and `MapLoader.tsx` as loaded-state-only/edge-state-unverified; separately own only `ReportPriceSheet.tsx` for the caution-status icon correction |
...
| Motion & Interaction Engineer | Retained/refuted two-path Market Details evidence lane; no new edit authorized | `NavigationStack.tsx` and `scripts/motion-contracts.test.ts` remain retained for the final combined compact rerun; no third timing edit and no `BottomSheet.tsx` claim |
...
| Contribution containment/integrity (`495750a`) | **Closed / released with controller runtime PASS** | The runtime artifact released `src/app/actions.ts`, `src/app/page.tsx`, `ReportPriceSheet.tsx`, `ConfirmVisitSheet.tsx`, and `src/core/i18n/strings.ts`. `page.tsx` later transferred to retained Market Details and `ReportPriceSheet.tsx` later transferred to the disjoint Iconography correction; Confirm Visit remains queued/unclaimed |
...
| Report Price unavailable-status icon correction | **Active, exact one-file disjoint Iconography lane** | Persistent Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac` owns only `src/app/_components/ReportPriceSheet.tsx`. Replace only the observed “Reports temporarily unavailable” bare `AlertTriangle` with a `status-caution` `IconOrb`/`SolidIcon`; preserve copy, status, state, actions, and ModalSheet Close. The identical `ConfirmVisitSheet.tsx` pattern is queued and unclaimed |
| Market Details — compact sticky Get-It containment | **Active exact one-file HI correction; related evidence paths retained/refuted** | Compact runtime diagnosis: `page.tsx` gives the sticky opaque Get-It CTA a negative bottom margin that removes its flow footprint while a large bottom offset pulls it upward, occluding the market title/location. Persistent Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d` owns only `src/app/page.tsx`; `PlaceOfferRow.tsx` remains retained, not released. Acceptance order: Back → market title/location → Get It → Prices; rows reachable; no root/document black escape; pop/reopen visibly resets; regular layout unchanged |
| Market Details level-1 scroll reset and containment | **Retained / runtime REFUTED; exact two evidence paths, no third timing edit authorized** | `src/design-system/components/NavigationStack.tsx` and `scripts/motion-contracts.test.ts` remain retained for the final combined compact rerun. Preserve the prior reset/containment contract, but authorize no further timing edit and do not claim `BottomSheet.tsx` |
...
### Active lane: Market Details — compact sticky Get-It containment
...
The independently agreed compact diagnosis is in this file: the sticky opaque Get-It CTA
uses a negative bottom margin that removes its flow footprint while the large bottom
offset pulls it upward, occluding the market title/location. Correct only that containment
in `page.tsx`. `src/design-system/components/PlaceOfferRow.tsx` remains retained for final
combined evidence and is not released or authorized for another edit.
...
Acceptance requires the compact sequence Back → market title/location → Get It → Prices;
all rows remain reachable; no root/document black escape occurs; pop/reopen visibly resets;
and regular layout remains unchanged.

### Retained evidence lane: Market Details level-1 scroll reset and containment

The following exact paths remain retained/refuted for the final combined compact rerun:

- `src/design-system/components/NavigationStack.tsx`
- `scripts/motion-contracts.test.ts`

The earlier runtime refutation remains open, but no third timing edit is authorized.
Re-run the reset/containment contract only after the one-file HI correction: level 1
visibly resets without resetting level 0, terminal scrolling does not escape to the
document, and push/pop animation, held-detail lifetime, focus/inert semantics, regular
layout, BottomSheet API, and one content instance remain intact.
`src/design-system/components/BottomSheet.tsx` is explicitly unclaimed.
...
### Active lane: Report Price unavailable-status icon correction
...
Replace only the observed “Reports temporarily unavailable” bare `AlertTriangle` with a
`status-caution` `IconOrb`/`SolidIcon`. Preserve the copy, status meaning, state, actions,
and ModalSheet Close. `src/app/_components/ConfirmVisitSheet.tsx` contains the identical
pattern but is queued and unclaimed; it is not part of this correction.
...
transferred to the retained/runtime-refuted Market Details scroll-containment evidence
lane with no third timing edit authorized. No caller,
...
| **Report Price unavailable-status icon correction** | 🟢 active exact one-file disjoint Iconography lane | persistent Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac`; independent visual/AX refuter | `src/app/_components/ReportPriceSheet.tsx` | Replace only the observed bare `AlertTriangle` for “Reports temporarily unavailable” with a `status-caution` `IconOrb`/`SolidIcon`; preserve copy/status/state/actions/ModalSheet Close. `ConfirmVisitSheet.tsx` is queued/unclaimed |
...
| **Market Details — compact sticky Get-It containment** | 🟢 active exact one-file HI correction; `PlaceOfferRow.tsx` retained for evidence | persistent Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; independent visual/accessibility refuter | Active: `src/app/page.tsx`. Retained: `src/design-system/components/PlaceOfferRow.tsx` | Correct the negative-margin/large-offset sticky CTA occlusion. Acceptance: Back → title/location → Get It → Prices; rows reachable; no document black escape; pop/reopen visibly resets; regular unchanged |
| **Market Details level-1 scroll reset and containment** | 🔴 retained/runtime REFUTED; no third timing edit authorized | Motion & Interaction Engineer; final combined compact refuter | `src/design-system/components/NavigationStack.tsx`; `scripts/motion-contracts.test.ts` | Retain for the combined rerun after the HI correction. Preserve reset/containment and interaction contracts; `BottomSheet.tsx` remains unclaimed |

The two retained evidence paths and the separate one-file Report Price correction are the
only Iconography ownership created by this table. The three matrix-PASS paths and Item
Detail/Get-It are released. Market Details remains a separate disjoint exact two-path
retained claim; its NavigationStack correction remains a disjoint two-path Motion claim.
...
| **Contribution containment and integrity** | controller runtime PASS; exact five paths released from Contribution | Contribution Integrity & Moderation Engineer; independent static/source and runtime evidence | Existing fail-closed containment plus a completed, unauthorized `0013+` planning packet covering ADR-019, schema/migration/manifest, app/ops paths, and dependencies | Runtime containment is closed. `page.tsx` later transferred to Market Details and `ReportPriceSheet.tsx` later transferred to the disjoint Iconography correction; Confirm Visit remains queued/unclaimed. The `0013+` packet remains planning-only behind separate Presence/shared-migration and activation gates. No DB/push/deploy |
| **Market Details — compact sticky Get-It containment** | ACTIVE exact one-file HI correction; related paths retained/refuted | persistent Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; independent visual/accessibility refuter | Active: `src/app/page.tsx`. Retained evidence: `src/design-system/components/PlaceOfferRow.tsx` | Correct the compact sticky CTA flow-footprint/offset occlusion. Require Back → title/location → Get It → Prices, reachable rows, no document black escape, visible pop/reopen reset, and unchanged regular layout |
| **Market Details level-1 scroll reset and containment** | RETAINED/runtime REFUTED; no third timing edit authorized | Motion & Interaction Engineer; final combined compact refuter | `src/design-system/components/NavigationStack.tsx`; `scripts/motion-contracts.test.ts` | Re-run only after the one-file HI correction; preserve prior reset/containment, push/pop, held detail, focus/inert, layout, BottomSheet API, and single-instance contracts. Do not claim `BottomSheet.tsx` |
...
| **Controlled Semantic Iconography bounded expansion** | foundation and Item Detail/Get-It released; intent-adoption matrix PASS for three released paths with two evidence paths retained; separate Report Price correction active | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac`; independent visual/accessibility refuters | Released: `SettingsSheet.tsx`, `CurrencyPickerSheet.tsx`, `ExchangePanel.tsx`. Retained for evidence: `SheetPicker.tsx`, `MapLoader.tsx`. Active correction: `ReportPriceSheet.tsx` only | SheetPicker runtime remains unverified; MapLoader edge states remain unverified after loaded-state evidence. Report Price changes only the observed unavailable-state bare warning to a status-caution orb; Confirm Visit is queued/unclaimed. No page/actions/map/DB |
...
| `src/app/page.tsx` | ~1300 | **Market Details compact sticky Get-It containment** — active exact one-file HI correction owned by Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d` |
| `src/design-system/components/PlaceOfferRow.tsx` | — | **retained/refuted evidence path** for final combined Market Details compact rerun; no new edit authorized |
...
| `src/design-system/components/NavigationStack.tsx`, `scripts/motion-contracts.test.ts` | — | **retained/runtime REFUTED** for final combined Market Details compact rerun; no third timing edit and no `BottomSheet.tsx` claim |
| `src/app/_components/ReportPriceSheet.tsx` | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac` | **Report Price unavailable-status icon correction** — active exact one-file claim |
| `src/app/_components/ConfirmVisitSheet.tsx` | — | **queued/unclaimed** for its identical bare unavailable-status pattern; not part of the Report Price claim |
...
| **Human Interface: category + Aboki FX flattening** | Human Interface employee; HI Quality refuter; Quality & Release Controller evidence custody | ✅ scoped visible PASS; completed paths released; combined release verdict remains REFUTED | — | Founder-directed Human Interface correction | 2026-07-18 | Selector/Aboki behavior passed visible desktop/compact and light/dark evidence. Safari keyboard focus and artificial loading/error/cache/empty states remain unproven. Those are residual release-evidence gaps, not retained implementation ownership. `CategorySelectorSheet.tsx` remains released; the later Iconography runtime matrix also passed and released `ExchangePanel.tsx`. |
...
| **Contribution containment plan** | Contribution Integrity & Moderation Engineer `019f75a3-f50d-7180-8e92-0a7aabd8a98c`; independent static/source refuter; HI Quality browser refuter | ✅ controller runtime artifact PASS; exact five paths released from Contribution | — | Immediate contribution safety containment | 2026-07-18 | The runtime artifact closed Contribution ownership. `page.tsx` later transferred to retained Market Details; `ReportPriceSheet.tsx` later transferred to the disjoint Iconography correction; `ConfirmVisitSheet.tsx` is queued/unclaimed. The `0013+` planning packet remains unauthorized behind separate Presence/shared-migration and activation gates. |
...
| **Market Details — compact sticky Get-It containment** | Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; independent visual/accessibility refuter | 🟢 active exact one-file HI correction; `PlaceOfferRow.tsx` retained | `src/app/page.tsx` active; `src/design-system/components/PlaceOfferRow.tsx` retained evidence | Founder-rejected compact market detail | 2026-07-18 | Negative bottom margin removes the opaque sticky CTA’s flow footprint while its large bottom offset pulls it upward over title/location. Require Back → title/location → Get It → Prices, reachable rows, no root/document black escape, visible pop/reopen reset, regular unchanged. |
| **Market Details level-1 scroll reset and containment** | Motion & Interaction Engineer; final combined compact refuter | 🔴 retained/runtime REFUTED; no third timing edit authorized | `src/design-system/components/NavigationStack.tsx`; `scripts/motion-contracts.test.ts` | Compact Market Details final combined evidence | 2026-07-18 | Retain both paths without another timing edit; rerun reset/containment and preservation contracts after the page-only HI correction. `BottomSheet.tsx` remains unclaimed. |
...
Hegel (`019f7645-cc25-7d42-b537-5a0eb37cda69`) confirmed the current market detail at `src/app/page.tsx` drops price range, availability, freshness, last-observed, image, and provenance semantics, and uses the artificial `max-h-[40vh]` dead zone. Commit `8b0d5e4` passed static refutation, but compact runtime refuted the integrated result. The independently agreed next diagnosis is narrower: `page.tsx` gives the sticky opaque Get-It CTA a negative bottom margin that removes its flow footprint while a large bottom offset pulls it upward over the market title/location. The persistent Human Interface Design Engineer owns only `src/app/page.tsx` for that correction. `PlaceOfferRow.tsx`, `NavigationStack.tsx`, and `scripts/motion-contracts.test.ts` remain retained/refuted for the combined compact rerun; no third NavigationStack timing edit is authorized. Require Back → market title/location → Get It → Prices, reachable rows, no root/document black escape, visible pop/reopen reset, and unchanged regular layout. No `BottomSheet.tsx`, generic category contract, DB, migration, push, or deployment work is authorized.
...
Market Details now authorizes the persistent Human Interface Design Engineer to edit only
`src/app/page.tsx`; `PlaceOfferRow.tsx`, `NavigationStack.tsx`, and
`scripts/motion-contracts.test.ts` remain retained/refuted for the final combined compact
rerun, with no third timing edit. H38, self-location/avatar, and Presence UI wait pathless
for `page.tsx`; H31 remains queued and unclaimed on released `actions.ts`.
...
1. **Market Details — compact sticky Get-It containment**: active exact one-file HI
   correction over `src/app/page.tsx`. `PlaceOfferRow.tsx` remains retained/refuted.
   Correct the negative-margin/large-offset occlusion and require Back → title/location →
   Get It → Prices, reachable rows, no document black escape, visible pop/reopen reset,
   and unchanged regular layout.
2. **Market Details level-1 scroll reset and containment**: NavigationStack and the motion
   contract remain retained/refuted for the final combined compact rerun. No third timing
   edit and no `BottomSheet.tsx` claim are authorized.
3. **Semantic iconography**: foundation and `ed5a6cb` Item Detail/Get-It are released. The
   HI runtime matrix releases Settings, Currency Picker, and Exchange Panel; retain only
   SheetPicker as runtime-unverified and MapLoader as loaded-state-only/edge-state-
   unverified. A disjoint active correction owns only `ReportPriceSheet.tsx` for the
   unavailable-state caution orb; Confirm Visit is queued/unclaimed.
