...
| Human Interface Design Engineer | Market Details runtime REFUTED; exact four paths retained | Commit `3fe637d` is pushed, but removing the negative margin did not resolve compact occlusion or document escape. Retain page, offer row, NavigationStack, and motion contract |
...
| Confirm Visit unavailable-status icon correction | **Active, exact one-file disjoint Iconography lane** | Persistent Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac` owns only `src/app/_components/ConfirmVisitSheet.tsx`. Replace only the identical unavailable-status bare `AlertTriangle` with a `status-caution` `IconOrb`/`SolidIcon` warning; preserve copy, role, state, actions, and neutral Close |
| Market Details — compact sticky Get-It containment | **Runtime REFUTED after `3fe637d`; exact four paths retained** | Removing the negative margin did not resolve the defect: title/location remain AX-only while pixels show Back → Get It → Prices; root/document black escape persists; reopen remains occluded. Retain `src/app/page.tsx`, `src/design-system/components/PlaceOfferRow.tsx`, `src/design-system/components/NavigationStack.tsx`, and `scripts/motion-contracts.test.ts` |
...
### Retained lane: Market Details — compact sticky Get-It containment

Implementation commit `3fe637d` is pushed, and the combined runtime verdict is REFUTED.
The following exact paths remain retained:
...
Removing the negative margin did not resolve the defect. The market title/location remain
available only to the accessibility tree while visible pixels order Back → Get It →
Prices; root/document black escape persists; and reopen remains occluded.
...
Implementation commit `b93761b` is pushed and the unavailable banner passed runtime:

- `src/app/_components/ReportPriceSheet.tsx`

The path is released.

### Active lane: Confirm Visit unavailable-status icon correction
...
Replace only the identical unavailable-status bare `AlertTriangle` with a
`status-caution` `IconOrb`/`SolidIcon` warning. Preserve copy, role, state, actions, and
the neutral Close. Do not touch Report Price, Market Details, navigation, data, database,
schema, migrations, browser, deployment, or another path.
...
| **Confirm Visit unavailable-status icon correction** | 🟢 active exact one-file disjoint Iconography lane | persistent Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac`; independent visual/AX refuter | `src/app/_components/ConfirmVisitSheet.tsx` | Replace only the identical unavailable-status bare `AlertTriangle` with a `status-caution` `IconOrb`/`SolidIcon`; preserve copy/role/state/actions/neutral Close |
...
| **Market Details — compact sticky Get-It containment** | 🔴 `3fe637d` runtime REFUTED; exact four paths retained | persistent Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; HI Regression | `src/app/page.tsx`; `src/design-system/components/PlaceOfferRow.tsx`; `src/design-system/components/NavigationStack.tsx`; `scripts/motion-contracts.test.ts` | Negative-margin removal did not resolve it: title/location AX-only, pixels Back → Get It → Prices, root/document black escape, reopen still occluded |
...
| **Market Details — compact sticky Get-It containment** | `3fe637d` runtime REFUTED; exact four paths retained | persistent Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; HI Regression | `src/app/page.tsx`; `src/design-system/components/PlaceOfferRow.tsx`; `src/design-system/components/NavigationStack.tsx`; `scripts/motion-contracts.test.ts` | Negative-margin removal did not resolve AX-only title/location, visible Back → Get It → Prices, document black escape, or reopen occlusion |
...
| **Controlled Semantic Iconography bounded expansion** | three matrix paths and Report Price runtime PASS/released; two evidence paths retained; Confirm Visit active exact one file | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac`; independent visual/accessibility refuters | Retained: `SheetPicker.tsx`, `MapLoader.tsx`. Active: `ConfirmVisitSheet.tsx` only. Released: `ReportPriceSheet.tsx` | Confirm Visit receives only the identical unavailable-state status-caution warning replacement while preserving copy/role/state/actions/neutral Close. No page/actions/map/DB |
...
| **Shared ModalSheet focus hardening** | completed at `f19990b`; Safari refuter PASS; ModalSheet released and contract later retained by Market runtime | Motion & Interaction Engineer / independent Safari refuter | Released: `src/design-system/components/ModalSheet.tsx`; retained: `scripts/motion-contracts.test.ts` | The contract path remains retained only for the combined Market Details runtime verdict |
...
| `src/app/page.tsx`, `src/design-system/components/PlaceOfferRow.tsx` | — | **retained/runtime REFUTED** after `3fe637d`; negative-margin removal did not resolve compact occlusion or document escape |
...
| `src/app/_components/ConfirmVisitSheet.tsx` | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac` | **Confirm Visit unavailable-status icon correction** — active exact one-file claim |
...
| **Market Details — compact sticky Get-It containment** | Human Interface Design Engineer `019f75a5-0fc6-7f40-9a0f-8097ead3b45d`; HI Regression | 🔴 `3fe637d` runtime REFUTED; exact four paths retained | `src/app/page.tsx`; `src/design-system/components/PlaceOfferRow.tsx`; `src/design-system/components/NavigationStack.tsx`; `scripts/motion-contracts.test.ts` | Founder-rejected compact market detail | 2026-07-18 | Title/location AX-only, visible pixels Back → Get It → Prices, root/document black escape, reopen occluded; negative-margin removal did not resolve it. |
...
| **Confirm Visit unavailable-status icon correction** | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac`; independent visual/AX refuter | 🟢 active exact one-file lane | `src/app/_components/ConfirmVisitSheet.tsx` | ADR-018 status warning adoption | 2026-07-18 | Replace only the identical bare `AlertTriangle` with a `status-caution` `IconOrb`/`SolidIcon`; preserve copy, role, state, actions, and neutral Close. |
...
1. **Market Details — compact sticky Get-It containment**: `3fe637d` is runtime REFUTED.
   Page, PlaceOfferRow, NavigationStack, and the motion contract remain retained. Removing
   the negative margin did not resolve AX-only title/location, visible Back → Get It →
   Prices, document black escape, or reopen occlusion.
...
3. **Semantic iconography**: foundation and `ed5a6cb` Item Detail/Get-It are released. The
   HI runtime matrix releases Settings, Currency Picker, and Exchange Panel; SheetPicker
   and MapLoader remain evidence-retained. Report Price `b93761b` passed/released; the
   active disjoint correction owns only `ConfirmVisitSheet.tsx`.
