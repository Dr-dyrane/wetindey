...
|---|---|---|---|
| **Semantic iconography foundation** | 🟢 exact bounded claim recorded; foundation precedes adoption | Iconography & Visual Systems Lead implementation owner; independent visual/accessibility refuter | `src/app/globals.css`; `tailwind.config.ts`; `src/design-system/components/IconOrb.tsx`; new `src/design-system/icons/SolidIcon.tsx`; `scripts/iconography-contracts.test.ts`. `package.json`/`package-lock.json` remain outside the claim unless the current auth owner releases them for a separately reviewed web-licensed solid-icon dependency | Preserve ADR-018 neutral controls and Food/Money/rating/status ownership; no blanket colour, generic/translucent-outline regression, Apple SF Symbols assumption, authentic-media/avatar change, push, or deploy |
| **Market Details Sheet Visual Systems** | 🟢 exact bounded claim recorded; separate commit/refutation unit after foundation | Iconography & Visual Systems Lead; independent visual/accessibility refuter | `src/app/_components/ItemDetailSheet.tsx`; `src/design-system/components/ListRow.tsx`; `src/design-system/components/SheetPicker.tsx`; `src/app/_components/GetItSheet.tsx` | Replace sparse/capped presentation with context-aware hierarchy, already-licensed/available item imagery, solid semantic icon orbs, comfortable vertical use, clear light/dark surfaces, and stable bottom action. Preserve truth semantics, item/offer counts, trust/provenance, accessibility, and fulfilment-out-of-scope |
| **Semantic iconography remaining sheet adoption** | 🟢 exact bounded claim recorded; separate from Market Details | same implementation owner; same independent visual/accessibility refuter | `src/design-system/components/MapLoader.tsx`; `src/app/_components/SettingsSheet.tsx`; `src/app/_components/CurrencyPickerSheet.tsx`; `src/app/_components/ExchangePanel.tsx` | Light/dark reused-state, accessible text/name/target, reduced-motion, and semantic-tone refutation required. Excludes currency actions/data, `CurrencyFlag`, flag sprite/licence, `page.tsx`, `actions.ts`, and Contribution paths |
| **Semantic map symbols** | 🟢 separate exact bounded claim recorded; no page wiring | same implementation owner; same independent visual/accessibility refuter | `src/design-system/components/MapboxCanvas.tsx`; `src/integrations/maps/MapboxAdapter.ts` | Preserve current marker contracts and H37 `placeType` forwarding; no self-avatar/peer-presence/product-page widening. Require light/dark visual and accessibility refutation before release |
...
| **H6 nonce-CSP implementation** | **BLOCKED before edit**; exact three-path claim retained, worker stopped | owner `/root/h6_nonce_implementation`; independent refuter `019f7692-d254-7e43-a687-154876274927` after any future commit | `src/middleware.ts` (new); `src/app/layout.tsx`; `vercel.json` | Repository evidence exposes only wildcard `*.public.blob.vercel-storage.com`, not the exact avatar store host ADR-020 requires, and contains no Preview CSP reporting endpoint/collector or approved redaction/access/retention contract. Do not guess the host, weaken policy, remove static CSP, or begin Preview report-only/enforcement. Zero code changes |
...
| **Account deletion reconciliation** | blocker confirmed; reported completion is not present in repository evidence; owns no paths | unassigned | Provider identity deletion plus approved app-PII/avatar handling, attribution/anonymisation, retention, and truthful completion semantics | No reachable branch, reflog entry, or inspected unreachable commit contains deletion implementation; no `deleteUser`/`deleteAccount` exists in current `src` or reachable history; Profile exposes Sign out only; privacy copy says deletion/retention remain under review. `708bc73` documents the blocker, not implementation. No deletion test/approval evidence found |
...
| **Controlled Semantic Iconography bounded expansion** | exact Foundation, Sheet Adoption, and Map Symbol claims activated; implementation pending | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac`; independent visual/accessibility refuter | Foundation must land before Sheet Adoption; Map Symbols remains a separate commit/refutation unit. Exact paths are locked in the ownership table below | Preserve ADR-018; require light/dark reused-state, accessibility, reduced-motion, licensed-web-icon provenance if any dependency is proposed, and no generic/translucent-outline regression. No product hot files, DB, push, or deploy |
...
| **Shared ModalSheet focus hardening** | `3e28c63` independently NOT REFUTED; path released | Motion & Interaction Engineer / independent refuter | Shared `isEnabledVisible` is used by initial focus and Tab trapping for native/ARIA disabled, hidden, disabled-fieldset, inert/ARIA-hidden, absent-layout, and computed display/visibility cases | Panel fallback, Escape, trapping, nesting, restoration, animations, and reduced motion are preserved. Browser/runtime focus, `visibility:collapse`, and restoration when the prior target becomes hidden remain residuals. Contribution retains its separate five paths |
...
| `src/app/_components/GetItSheet.tsx` | — | **released** after `69737de` exact-scope independent NOT REFUTED with no P1/P2 and focused contract PASS |
...
| `src/design-system/components/ModalSheet.tsx` | — | **released** after `3e28c63` independently NOT REFUTED; browser/runtime focus and narrow visibility/restoration residuals remain |
...
| `src/db/seed.ts` | Catalog Stewardship Engineer `019f75a3-f38d-7893-9b82-2d6871a2563c` | **H27 seed-trust correction** — active exact one-file Stage 0 claim; no seed execution/reseed/DB; independent refuter `/root/h27_seed_trust_refuter` required |
...
| `src/middleware.ts`, `src/app/layout.tsx`, `vercel.json` | `/root/h6_nonce_implementation` | **H6 nonce-CSP implementation BLOCKED before edit** — exact claim retained; missing exact Vercel Blob avatar host and Preview collector/redaction/access/retention contract; worker stopped with zero changes; independent refuter `019f7692-d254-7e43-a687-154876274927` required after any future commit |
| `src/app/globals.css`, `tailwind.config.ts`, `src/design-system/components/IconOrb.tsx`, `src/design-system/components/ListRow.tsx`, `src/design-system/components/ItemCard.tsx` | — | **released** after `69737de` exact-scope independent NOT REFUTED with no P1/P2 and focused contract PASS |
| `src/app/globals.css`, `tailwind.config.ts`, `src/design-system/components/IconOrb.tsx`, new `src/design-system/icons/SolidIcon.tsx`, `scripts/iconography-contracts.test.ts` | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac` | **Semantic iconography foundation** — active exact claim; one local commit then independent visual/accessibility refutation |
| `src/app/_components/ItemDetailSheet.tsx`, `src/design-system/components/ListRow.tsx`, `src/design-system/components/SheetPicker.tsx`, `src/app/_components/GetItSheet.tsx` | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac` | **Market Details Sheet Visual Systems** — active exact claim; separate commit and light/dark/accessibility/reduced-motion refutation after foundation |
| `src/design-system/components/MapLoader.tsx`, `src/app/_components/SettingsSheet.tsx`, `src/app/_components/CurrencyPickerSheet.tsx`, `src/app/_components/ExchangePanel.tsx` | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac` | **Remaining semantic sheet adoption** — active exact claim, separate from Market Details |
| `src/design-system/components/MapboxCanvas.tsx`, `src/integrations/maps/MapboxAdapter.ts` | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac` | **Semantic map symbols** — separate active exact claim; preserve H37 contracts; no page/presence widening |
...
| `scripts/iconography-contracts.test.ts` | — | **released source** after its focused contract passed for `69737de`; runtime visual and independent contrast evidence remain separate |
...
| **H27 seed-trust correction** | Catalog Stewardship Engineer `019f75a3-f38d-7893-9b82-2d6871a2563c`; independent refuter `/root/h27_seed_trust_refuter` | 🟢 active exact one-file Stage 0 claim | `src/db/seed.ts` | Remove synthetic freshness-to-trust ladder | 2026-07-18 | Synthetic/Sample offers must not gain observed-evidence confidence from age. Use canonical derived semantics or a justified conservative schema-compatible synthetic value. Preserve deterministic PRNG, one run timestamp, catalog/slugs/variants, consumer-primary ordering, Spaghetti contract, freshness/availability timestamps, synthetic provenance, and Sample wording. No `actions.ts`, `lib/trust.ts`, `seedContent.ts`, schema, migration, UI, test-path, seed execution, connected DB, push, or deploy. One path-scoped commit then default-to-refuted static/data-trust review. |
...
| **H6 nonce-CSP implementation** | owner `/root/h6_nonce_implementation`; independent refuter `019f7692-d254-7e43-a687-154876274927` | 🔴 BLOCKED before edit; exact three paths retained; worker stopped | `src/middleware.ts`; `src/app/layout.tsx`; `vercel.json` | ADR-020 implementation | 2026-07-18 | Repository-visible Blob evidence is only `*.public.blob.vercel-storage.com` plus runtime `blob.url`; no exact per-store avatar host is recorded, and ADR-020 forbids guessing the wildcard. No reporting endpoint/header, collector implementation, or approved Preview redaction/access/retention contract exists. Preview report-only, enforcement, and static-CSP removal remain prohibited. Zero edits/tests/build/browser/shared-environment access/push/deploy. |
| **Account deletion evidence reconciliation** | Quality & Release Controller evidence custody | 🔴 reported completion not found; provider/counsel blocker remains | — | Identity/privacy release gate | 2026-07-18 | `git log --all`, all visible refs, reflog, and inspected unreachable commit subjects show no deletion implementation. Current/reachable history contains no `deleteUser` or `deleteAccount`; Profile offers Sign out only; `about.privacy_delete` directs support and says deletion/retention are under review. Commit `708bc73` records unresolved Neon capability evidence. No deletion test, destructive authenticated proof, counsel retention matrix, or approval receipt exists in repository evidence. |
...
| H27 | **seed-only correction active with Catalog Stewardship Engineer `019f75a3-f38d-7893-9b82-2d6871a2563c`; stale-comment follow-up remains queued** | Exact active path is only `src/db/seed.ts`, which still stamps a hardcoded freshness-to-trust ladder. Stale deleted-`getFoodItemCandidates` comments in `src/lib/trust.ts` and Contribution-owned `src/app/actions.ts` are explicitly outside this claim. Connected seeded rows and destructive refresh remain separate DB gates. | Current HEAD: `src/db/seed.ts` `trustLevel` assignment; independent refuter `/root/h27_seed_trust_refuter`. | **ACTIVE one-file source correction; no reseed/DB authority. Comment cleanup remains unclaimed and `actions.ts`-blocked.** |
...
Disposable proof remains a separate future authorization and must identify a guarded
disposable target; reproduce exact migration lineage through `0012`; prove required
...
presence. A named safety responder and approved legal/privacy notice are mandatory before
pilot traffic. No shared migration, pilot, push, or deployment is authorized.
...
The former Motion diagnosis worker is no longer addressable. Mendel (`019f7638-4f36-7952-b775-e9b141f0fd7d`) diagnosed the shared lifecycle defect; Carver implemented `f7cddba`, Leibniz refuted disabled-fieldset ancestry, and Godel corrected it in `9fe9d4e`. Final candidate `3e28c63` uses shared `isEnabledVisible` for initial focus and Tab trapping and independently returned NOT REFUTED. It covers native/ARIA disabled, hidden, disabled fieldsets, inert/ARIA-hidden, absent layout, and computed display/visibility while preserving panel fallback, Escape, trapping, nesting, restoration, animations, and reduced motion. `src/design-system/components/ModalSheet.tsx` is released. Browser/runtime focus, `visibility:collapse`, and restoration when the prior target becomes hidden remain residuals; the five Contribution paths remain protected.
