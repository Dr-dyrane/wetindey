...
#### Dead profile-sheet copy module removal - ACTIVE

Owner: Private Contractor, Maps Delivery `c9c17443-ef5e-4a7b-9b6e-c8f5381da30c`; the O3 residual from the auth sweep. profile-sheet/copy/copy.ts has zero importers (the sign-in view renders via the central i18n auth.* keys) and its strings do not even match the live copy ("Send OTP Code" versus the shipped "Send code"), so it is dead weight that could drift into a second source of truth. Delete it and add profile-sheet to the modularization contract's CENTRALIZED_COPY set exactly as location-sheet and confirm-visit-sheet already are (its i18n read lives in the view, which the tooth greps). No runtime change: nothing imports the module. Exact paths: delete `src/app/_components/profile-sheet/copy/copy.ts`; `scripts/modularization-structure-contract.test.ts`. Completion: modularization contract green with profile-sheet exempted and its no-copy-dir and i18n-read teeth proven, gates green, independent default-to-REFUTED refutation, path-scoped commit, release.

#### Auth-email injection-guard contract hardening - RELEASED / PATHLESS

Complete at the contract commit, pushed under the `0bbdb11` class after all four claims confirmed: scripts-only (no runtime file touched), the three cases exercise the real exports (verifyNeonOtpWebhook over a genuinely signed body, renderWetinDeyOtpEmail, sendWetinDeyOtpEmail), and all three teeth bite in an out-of-repo mutant mirror (neuter the recipient validator, escapeHtml, or the from/replyTo CRLF disjunction and the intended new assertion fails), with no overreach or duplication. The anti-injection surface is now pinned so a future loosening of the numeric OTP validator cannot silently drop the last defense. The broader auth subsystem was found otherwise clean; posture items C2/O1/O2/O4 stay routed to a security/auth seat below. Paths released.

#### ROUTED to a security/auth seat: auth-email posture items (from the same sweep)

Not claimed; these need an auth-owner decision or a durable store the Maps seat should not choose unilaterally. (C2) The OTP delivery/replay dedup (`deliveryCache`, `cachedJwks` in `src/lib/auth-email.ts`) is module-scoped in-memory, so on serverless cold starts Neon's own 1-3 delivery retries and in-window webhook replays dedup only within a warm instance; the same OTP email can send 2-3 times across cold starts (duplicate email, not an auth bypass, signature verification holds). A true idempotency barrier needs a durable store. (O1) No app-layer rate limit on the initial sendCode; the posture is entirely delegated to Neon, and the UI honestly surfaces a 429 but has no defense if Neon does not enforce. (O2) Error mapping reads better-auth's `.status`/`.code` shape (`useProfileSheet.ts`), so an SDK error-shape drift silently degrades 429/OTP_EXPIRED detection to the generic copy. (O4) Igbo leaves all `auth.*` strings UNTRANSLATED and Pidgin leaves two; English falls through honestly, but completing the coverage needs owner-supplied native copy, never agent-invented.

#### ROUTED to Contribution Integrity seat: moderation-runtime hardening (from a read-only sweep)

Not claimed by the Maps seat; the moderation operations vertical (`a64b623`/`b700acb`) is Contribution Integrity's. A read-only sweep surfaced findings for that seat, ranked; finding 1 is production-reachable today and should be prioritized:

1. UNVALIDATED WIRE INPUT 500s EVEN WITH THE FLAG OFF. `moderateContribution` (`src/lib/contributions/moderation-runtime.ts:175`) calls `isReasonCode(command.decision, ...)` before the env gate at :176 and before any try; an attacker-chosen key (`"constructor"` returns Function, unknown returns undefined) makes `.includes` throw an uncaught TypeError, and `command` itself may be null from the deserialized server-action payload (`src/app/operations/contributions/actions.ts`). Every other path in this runtime fails closed with a result code; this one 500s with a digest, reachable in production despite the flag being off. Fix: validate `decision` is a known key and `command` is an object before the lookup, returning the runtime's own fail-closed code.
2. ERROR-CODE DISHONESTY. `moderation-runtime.ts:161,172,188` collapse every post-probe error to `forbidden` ("This account is not assigned to contribution review"), so a lost decide race (SQL 23514), the DB kill switch, self-moderation rejection, and any transient DB failure all render as not-assigned; the existing conflict copy ("This review changed. Refresh") is unreachable for the most common race. Map 23514/kill-switch/transient to distinct codes.
3. CONFLICT DETECTION COUPLED TO UNPINNED SQL PROSE. The `/idempotency|request.*different|payload digest/i` regex (`:188`) must match `src/db/pillars/80-contribution-services.sql:493`; the migration contract that exercised this end-to-end is archived, so a wording edit silently downgrades all idempotency conflicts to `forbidden`. Pin both strings in a live contract.
4. A11y: the console's `role="status"` live region is conditionally mounted (`ModerationConsoleView.tsx:27`) so outcomes may never announce; decision success drops focus to body (`useModerationConsole.ts:85`); no focus management anywhere. Reject/reverse are one-tap with no confirmation.
5. Contract coverage: only the reverse reason array is fully pinned; the reject vocabulary is pinned nowhere while SQL accepts any `^[a-z0-9_]{2,64}$`, so a vocabulary edit silently changes the button set. Audit history mislabels non-moderator actors as `another_moderator` (`:152`). ADR-019 conformance itself is clean: flag fail-closed, no activation flipped, DB kill switch present, public projections untouched.

#### Image delivery repairs: review avatars and item hero - RELEASED / PATHLESS

Complete at `e37ba45`, pushed under the `0bbdb11` class. The production avatar block was independently confirmed (blob host absent from both CSPs' img-src, present in remotePatterns, no error exit at HEAD) and the fix proven without touching either CSP: SSR markup with all seventeen srcset candidates on /_next/image and a live DOM error-event flipping to initials. The hero comparison reproduced the LCP warning at HEAD and its absence at candidate, with the preload emitted. The refutation killed one dead contract tooth (a comment word satisfying the priority assertion), which was fixed to prop-form matching and the identical surgical mutation re-proven failing; a third dead tailwind glob the refuter flagged was dropped in the same pass and the file's pre-existing lint error removed via the ESM plugin import, eslint now genuinely clean on all six paths. Function region pinning remains routed to the controller (depends on where Neon lives). Paths released.

#### Confirm-visit lang prop drop - RELEASED / PATHLESS

Complete at the drop commit, pushed under the `0bbdb11` class after NOT REFUTED: zero lang references remain in the vertical or at the call site with SettingsSheet's consulted lang untouched, the boot-critical block and central Copy resolution byte-identical, both contracts and the static stage green, and the refuter re-drove the full arm and consume flow live (22 checks, central copy in en and pidgin, zero errors). The fork-merge lane's last residual is closed. Paths released.

#### Docs-truth wave 4: product-doc path refresh - RELEASED / PATHLESS

Complete at `98caf1f` plus the strings.ts follow-up commit, pushed under the `0bbdb11` class after NOT REFUTED on all four claims: sixteen product-doc citations verified resolving at HEAD with the dated audit's analysis preserved, the BIBLE's three normative-hazard sentences marked with pure insertions pointing at the SERVICE-ARCHITECTURE amendment, and no overclaim in any marker (the roles resolver marker verified as implying no wiring). The wave's own side finding, four strings.ts comments made stale one day earlier by the fork-merge deleting the file they cite, was refuter-grounded by line number, fixed comment-only (token-stream identity proven), and delta-confirmed. Recorded residuals: one grammatical-glue word in the owner-profile row and the relocation-versus-split lineage nuance. The docs-truth program is now fully dispositioned across all four waves plus the ADR-006 amendment. Paths released.

#### Confirm-visit copy fork merge - RELEASED / PATHLESS

Complete at the merge commit, pushed under the `0bbdb11` class after NOT REFUTED on all five claims: parity independently proven 39 of 39 cells byte-identical (so strings.ts was rightly untouched, zero reconciliations), 51 interpolated resolutions byte-identical across the three locales, the boot-critical arm and consume block byte-identical, and the refuter drove the real machinery live (a young arm withheld, a due arm opening the sheet with central copy in en and pidgin, an expired arm discarded, zero errors). Four contract mutations bit, including proof the CENTRALIZED_COPY entry is load-bearing. Residual recorded: the sheet's lang prop is retained but no longer consulted; dropping it touches HomePageView and is a later micro-lane. Paths released.

#### ADR-006 second amendment - RELEASED / PATHLESS

Executed from the routed docs-truth list by the Maps seat, complete at the amendment commit under the `0bbdb11` class. The first draft was genuinely REFUTED on four fresh rot instances (a nonexistent file described as a barrel, misplaced assessTrust call sites, a seed ladder that is now a flat none, and a dead presenter symbol), rewritten strictly from the refuter's measurements, and confirmed sentence by sentence: the old verification grep errors on a missing file, the read path is the sole assessTrust home alongside seo-queries, write-path trust derives in SQL via contribution_admit, and the corrected grep returns 0. Insert-only above the 2026-07-17 block. The lesson stands recorded: docs-truth prose is written from measurement, never memory. Paths released.

#### Docs-truth conformance wave 3: source-comment citation rot - RELEASED / PATHLESS

Complete at the citation-conversion commit, pushed under the `0bbdb11` class. Comment-only proven by TypeScript token-stream identity per file; 21 added citations verified resolving by the refuter, which then genuinely REFUTED the wave on a phantom SQL name minted while curing rot (contribution_submit for contribution_admit, twice), the wrong deletion commit for the getFoodItemCandidates history (f06dc1b, not the monolith split), and a self-contradiction the wave left in trust.ts's own edited file; all fixed with the refuter's pre-existing pill-class nit, and delta-confirmed. Two claims were found more drifted than the sweep recorded: the reliability mint is 70 not 75, and the placesNear and coverage parsers now enforce what their comments called aspirational. Remaining routed: ADR-006 second amendment, the maps-location forward entry, the human-interface dead-command action (HI seat), product-doc path refresh, and a recorded candidate lane for merging the confirm-visit COPY fork into the central dictionary (its vouched local tables move verbatim, no invention). Paths released.

#### Docs-truth conformance wave 2: SERVICE-ARCHITECTURE amendment - RELEASED / PATHLESS

Complete at `02938d6`, pushed under the `0bbdb11` class. The refuter independently verified all five amendment statements (exact four-caller set for the internalized trust impl, the twelve-line page shell and eleven action files, both selectedPlaceId consumption symbols, 27 i18n importers, the state-library cut) and proved the diff pure insert-and-append with every original line restoring byte-exactly when markers strip; the single refuted element (the second trust table row lacked the stronger do-not-execute marker) was fixed before landing. The dangerous R2 wire-this-first directive now cannot be read without its supersession. Remaining from the sweep, still routed: source-comment file+symbol citation conversion (wave 3), ADR-006 second amendment, the maps-location forward entry, the human-interface dead-command action (HI seat), and the product-doc path refresh. Paths released.

#### Docs-truth conformance wave 1 - RELEASED / PATHLESS

Complete at `c70aa63`, pushed under the `0bbdb11` class after a refutation that earned its keep twice. Round one verified every corrected status claim against the tree and history (including that ADR-020 is report-only, P3 genuinely absent, roles has no live caller, evidence-media default-off, and the four README env keys match .env.example) but REFUTED two fresh README drifts introduced by the lane itself; round two REFUTED the fix again on a spread-shadowing bug in run-stage.mjs, which exposed that the static stage had silently dropped the contribution-actions contract since `8fbd259` because this seat's earlier hardening-contract registration spread into that entry. The stage is now three properly labeled entries, runs 16 commands all green, and enforces the roles contract for the first time; run-stage.mjs joins the lane record as an amendment under the registration precedent. Remaining waves stay routed: SERVICE-ARCHITECTURE amendment block (wave 2), source-comment file+symbol citation conversion (wave 3), the maps-location forward entry, and the human-interface dead-command action (HI seat). Paths released.

#### Read-hygiene riders: robots rule, sitemap N+1, trust freshness window - RELEASED / PATHLESS

Complete at `959839e`, pushed under the `0bbdb11` class after NOT REFUTED on all four claims. The dead /zzboom robots rule is gone with everything else byte-unchanged. The sitemap's serial per-row evidence probe is a bounded-concurrency map, cap 8: rendered output proven identical at HEAD versus candidate, a 300-item adversarial driver measured peak in-flight exactly 8 with order preserved, and rejection propagation proven equivalent. The trust freshness window was REFUTED BY THE LANE ITSELF and the ruling shipped instead: the refuter independently confirmed no constant both bounds the read and preserves output (a freshness-scale window flips about 95 percent of keys from Sample to No observed reports day one; an oversized one breaks silently as rows age past it), the ruling is recorded on the read and contract-pinned so any future window must confront it, and the growth bound belongs to the routed retention decision. Two prose nits from the refuter (a perishable row count, one over-broad sentence) fixed before landing. Three mirror mutations bit including a serial-loop bypass variant. Paths released.

#### Server-action read hardening - RELEASED / PATHLESS

Complete at `8fbd259`, pushed under the `0bbdb11` class after NOT REFUTED on five claims with independent runtime reproduction of all three defects at HEAD (the open trust endpoint answered fabricated keys; a junk review id round-tripped to a postgres uuid error; getPopularItems(8) produced the cohort-query syntax error). Trust module internalized per ADR-002/ADR-006 with logic byte-identical; review reads parse-first with LIMIT 200 and an injection-shaped id rejecting cleanly; the legacy positional signature is gone with zero callers broken; category is a parsed slug. Parser parity proven on boundaries; well-formed results byte-identical HEAD versus candidate; 4/4 mirror mutations bit. The refuter's one finding, an unregistered contract, was fixed by registering it in the static quality stage. M4 RULING ROUTED TO CONTROLLER: the shared-applied 0019 release manifest byte-pins src/app/_actions/evidence-media-actions.ts and the CI evidence-media contract enforces the pin, so the uuid-parse guards for that file wait on either a manifest source_sha256 amendment policy or the next release-manifest cycle. Paths released.

#### Contract-test rot repair - RELEASED / PATHLESS

Complete at `b094dda`, pushed under the `0bbdb11` class after a refutation SUSTAINED on all three claims: each of the four CI-orphaned tests reproduced its exact rot at HEAD by stash round-trip, all four now green under plain npx tsx, and no assertion weakened (motion's focus pin is stronger; modularization's centralized-copy exemption carries two new mutation-proven teeth; the vocab regex still anchors and both its teeth bite; moderation-runtime's server-only import is unweakened, proven by the condition-less run still throwing). The pre-existing subEntries eslint warning predates the lane. Paths released.

Routed to controller from sweep two, not claimable until decided: (5) wire the twenty CI-orphaned contract tests into the static quality stage (adds seconds; which belong is a gate-policy call); (6) migration timing for two missing observation indexes, observations.source_id (every My-Reports open scans) and observed_at (sitemap and search windows), which need journal sequencing per the shared-tree rules; (7) the nonce-versus-ISR conflict already routed above now has sweep-two evidence that JsonLd's headers() read alone forces dynamic.
...
Complete at `bcf6028`, pushed under the `0bbdb11` class after a refutation that earned its keep: the refuter independently re-derived the permission vocabulary and transition table from the ADR and matched them, ran its own 21-case adversarial deny suite with zero allows and Proxy proof the stale claim is never read, then genuinely REFUTED the first candidate on appeal-flag wear (an ordinary transition defaulted appealUsed to false and re-armed the single bounded appeal) and flagged a mutating-getter TOCTOU residual; both were fixed in the refuter's prescribed shapes (ordinary patches never carry appealUsed; scope refs parsed read-once into locals) and delta-confirmed twice, all four mutation teeth still biting, contract 27 of 27. The schema and `0020` half remains the separate CLAIM REQUEST above, still awaiting the two controller rulings; its pgEnums will cross-check these tuples when granted. No live caller imports src/lib/roles. Paths released.
