...
| Iconography & Visual Systems Lead | Active, exact two-path Item Detail/Get-It adoption | Deliver the first live `SolidIcon` adoption and semantic unavailable `IconOrb` over only `ItemDetailSheet.tsx` and `GetItSheet.tsx`; foundation paths are released |
...
### Active lane: Iconography solid-glyph adoption — Item Detail/Get-It
...
Deliver the first live `SolidIcon` adoption. Replace Item Detail's private translucent
unavailable wrapper with semantic `IconOrb`; use stable contextual families for
navigation, contact, rating, and status icons; and keep universal Close neutral. Preserve
all copy, data, actions, layout, and motion. Do not touch page, actions, maps, database,
schema, or migrations. Acceptance requires an existing-tab light/dark visual and
accessibility refutation followed by one path-scoped commit.

Settings/`SheetPicker`/`CurrencyPickerSheet`/`ExchangePanel` adoption remains a separate
pending unclaimed lane.
...
| **Iconography solid-glyph adoption — Item Detail/Get-It** | 🟢 active exact two-path lane | persistent Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac`; existing-tab visual and AX refuter | `src/app/_components/ItemDetailSheet.tsx`; `src/app/_components/GetItSheet.tsx` | First live `SolidIcon` adoption; semantic unavailable `IconOrb`; stable contextual navigation/contact/rating/status families; universal Close neutral; preserve copy/data/actions/layout/motion. No page/actions/map/DB |
...
The active two-path Item Detail/Get-It row above is the only Iconography ownership created
by this table. Remaining adoption rows are roadmap/triage direction only and grant no
path. Market Details is a separate disjoint exact three-path active claim.
...
| **Controlled Semantic Iconography bounded expansion** | foundation runtime PASS/released; Item Detail/Get-It adoption ACTIVE exact two paths | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac`; independent visual/accessibility refuters | Released foundation: `globals.css`, `tailwind.config.ts`, `IconOrb.tsx`, `SolidIcon.tsx`, `iconography-contracts.test.ts`. Active adoption: `ItemDetailSheet.tsx`, `GetItSheet.tsx` | Foundation existing-tab light/dark/grayscale and focused contract passed; forced-colors emulation remains residual. Adoption must deliver first live SolidIcon and semantic unavailable IconOrb while preserving copy/data/actions/layout/motion; existing-tab light/dark plus AX refutation. No page/actions/map/DB |
...
| `src/app/_components/ItemDetailSheet.tsx`, `src/app/_components/GetItSheet.tsx` | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac` | **Iconography solid-glyph adoption — Item Detail/Get-It** — active exact two-path lane; preserve copy/data/actions/layout/motion; existing-tab light/dark visual plus AX refutation required |
...
| `src/app/_components/ItemDetailSheet.tsx`, `src/app/_components/GetItSheet.tsx` | Iconography & Visual Systems Lead `019f75cf-4d8f-7263-822b-06e846fce7ac` | **Iconography solid-glyph adoption — Item Detail/Get-It** — active exact two-path claim; no page/actions/map/DB widening |
...
| `src/design-system/components/MapLoader.tsx` | — | **MapLoader iconography adoption** — pending and unclaimed; no edit authority without a fresh exact handoff |
...
CEO sequencing checkpoint: the iconography foundation passed and released, with its
two-path Item Detail/Get-It adoption active and disjoint. Market Details is now active for
...
2. **Semantic iconography**: foundation runtime/focused-contract PASS released all five
   paths. The persistent Iconography Lead now actively owns only
   `ItemDetailSheet.tsx` and `GetItSheet.tsx` for first live SolidIcon adoption and the
   semantic unavailable IconOrb; existing-tab light/dark visual plus AX refutation is
   required.
