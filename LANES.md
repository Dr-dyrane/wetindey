...
#### Confirm-visit copy fork merge - ACTIVE

Owner: Private Contractor, Maps Delivery `c9c17443-ef5e-4a7b-9b6e-c8f5381da30c`; the recorded candidate from docs-truth wave 3. The central dictionary's header says the confirm-visit strings were reproduced verbatim under confirm.* from the sheet's own COPY module because its Yoruba was the better source, yet the sheet still reads the local fork: two dictionaries, one product, exactly what strings.ts exists to end. The merge repoints the sheet at the central confirm.* keys and deletes the fork, with byte-parity between the two tables proven per key and locale BEFORE the repoint; any divergence found is resolved toward the vouched local fork's value (updating the central key verbatim, no invention) and recorded. Exact paths: `src/app/_components/confirm-visit-sheet/` (its copy module deleted and consumers repointed); `src/core/i18n/strings.ts` (only if parity requires verbatim reconciliation); extend `scripts/i18n/copy-hoist-contract.test.ts`; `scripts/modularization-structure-contract.test.ts` (CENTRALIZED_COPY gains confirm-visit-sheet). The boot-critical armVisit/takeDueVisit statics are copy-independent and must be byte-untouched. Completion: rendered copy byte-identical in all three locales, contracts green, gates green, independent default-to-REFUTED refutation, path-scoped commit, release.
