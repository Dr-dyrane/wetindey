...
#### Confirm-visit lang prop drop - ACTIVE

Owner: Private Contractor, Maps Delivery `c9c17443-ef5e-4a7b-9b6e-c8f5381da30c`; the residual recorded by the released fork-merge lane. The sheet's lang prop is retained but no longer consulted (the hook reads the central locale store, and the prop's only live feeder passes that same store's value); dropping it removes a decoy API that invites a caller to expect a per-sheet language override the shippability gate would not honor. Exact paths: `src/app/_components/confirm-visit-sheet/ConfirmVisitSheet.tsx`; `src/app/_components/confirm-visit-sheet/hooks/useConfirmVisitSheet.ts`; `src/app/_components/home-page/views/HomePageView.tsx` (remove the sheet's lang pass-through only; the SettingsSheet lang stays, it is consulted). Completion: tsc silent, contracts green, refuter two-line confirmation, path-scoped commit, release.
