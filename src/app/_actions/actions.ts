...
import { db } from "@/db";
import { areas, items, itemAliases, itemVariants, places, offersCurrent, units, observations, sources, userProfiles, reviewAggregates } from "@/db/schema";
import { eq, ilike, or, and, sql, isNull, asc, desc } from "drizzle-orm";
...
// 5. Submit crowdsourced food price observations through the ADR-019 boundary
export async function submitObservation(
  data: ContributionAdmissionInput
): Promise<ContributionAdmissionResult> {
  // The delegated boundary checks its exact release gate before it resolves
  // request context or creates a connection. Keep this as the first statement.
...
/** Visit confirmation remains outside Phase 2A and fails closed. */
 ...
 * there is nothing for `src/lib/validation.ts` to gate. Contribution admission
 * also resolves its actor from the session rather than accepting an account id.
 *
 ...
 * offers (there is no `is_latest` column, this is a derived predicate). New
 * Phase 2A reports remain pending and therefore do not change this legacy read.
 */
 ...
 *
 * SESSION RESOLVED SERVER-SIDE, NO ARGUMENTS, exactly like `getMyReports`. A
 * Server Action is a public HTTP endpoint; one that
 ...
 * `auth.getSession()` exactly as `getMyReports` reads it; the payload carries
 * only the contact fields, so there is no id to tamper
