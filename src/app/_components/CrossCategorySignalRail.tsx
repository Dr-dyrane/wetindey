...
export function CrossCategorySignalRail({
  ...
  return (
    <button
      type="button"
      className="group grid h-11 min-w-0 flex-1 place-items-center px-0.5 text-left active:scale-[0.96] transition-transform duration-150"
    ...
    >
      <span className="relative flex h-[36px] w-full min-w-0 max-w-[210px] items-center justify-between overflow-hidden rounded-[14px] bg-fillSecondary px-2.5 shadow-sm transition-all hover:bg-fillTertiary">
        <span
          key={signal.id}
        ...
        >
          ...
          {signal.visual === "usd" ? (
            <CurrencyFlag code="USD" className="h-4 w-4 shrink-0" />
          ) : (
            <Utensils
          ...
          <span className="hidden min-w-0 truncate text-caption-1 font-bold tracking-tight text-text-primary min-[370px]:block">
            {signal.shortLabel}
          </span>
          <span className="min-w-0 truncate text-caption-1 font-bold tracking-tight text-text-primary min-[370px]:hidden">
            {signal.compactLabel}
          </span>
        </span>

        <ChevronRight className="h-3.5 w-3.5 shrink-0 text-text-tertiary transition-transform group-hover:translate-x-0.5" aria-hidden />
      </span>
    </button>
  );
}
