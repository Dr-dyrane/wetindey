...
| [020](docs/adr/020-per-request-nonce-content-security-policy.md) | Per-request nonce Content Security Policy | Accepted — architecture only | **One request-boundary nonce policy reaches Next 15.5.20 through identical request and response headers.** A later enforcement change removes the static Vercel CSP in the same commit, nonces all raw layout scripts and Mapbox, makes HTML dynamic/private/no-store, and forbids duplicate enforcement. |
| [021](docs/adr/021-account-deletion-lifecycle.md) | Account deletion lifecycle | Accepted — policy; implementation unclaimed | **Deletion is a persisted idempotent saga:** fresh OTP/re-auth, server-only exact-branch Neon Auth deletion, then retryable profile/attribution/problem-report/Presence/Blob cleanup. Observations remain unchanged; completion requires every phase receipt. |
| [022](docs/adr/022-earned-seller-and-role-onboarding.md) | Earned seller stewardship and extensible role onboarding | Accepted — architecture only; implementation unclaimed | **Application-owned, provider-independent, scoped RBAC keeps Auth, business proof, place control, contact consent, accuracy, confidence, and badges separate.** Seller roles, moderation/appeal/audit, and revocable contact publication follow Food truth and contribution-integrity migrations. Identity/business verification, place-control approval, roles, lifecycle/status, accuracy/ranking, and badges cannot be purchased. |
| [023](docs/adr/023-browsing-context-and-device-location.md) | Browsing context and device location are separate | Accepted — architecture only; implementation unclaimed | **Browsing context, device location, camera centre, and selected place are four different concepts.** Default/manual/simulated browsing points never become `Me`, personal avatar, exact route origin, or Presence evidence. Valid device fixes retain accuracy and capture time even outside coverage; exact origin egress requires a fresh fix and explicit action-stage disclosure. |
| [026](docs/adr/026-liquid-glass-sheet-material-system.md) | Liquid Glass sheet material system | Accepted — visual architecture only; implementation separately evidenced | **Named translucent sheet materials use `backdrop-blur-sm` (~8px), restrained/no saturation, context islands ~58–64% alpha, dense docked/modal/expanded glass ~70–82%, one blur layer per visible sheet, no nested blur/card slabs, subtle inset edge light/elevation, no decorative borders, and transform/opacity-only animation. Opaque slabs are prohibited except reduced-transparency or forced-colors accessibility fallbacks; unsupported or failed performance blocks acceptance.** |
| [027](adr/027-ai-routed-general-search.md) | AI-routed General Search over typed truth capabilities | Accepted — architecture only; implementation separately claimed | **AI interprets a validated natural-language request; only an allowlisted live WetinDey capability answers from admitted evidence through deterministic rendering.** ADR-010's two-complete-vertical abstraction gate, explicit category fallback, privacy minimization, schema validation, abstention, and deterministic failure remain required. No endpoint, provider, registry, schema, UI, deployment, or rollout is authorized by acceptance alone. |
...
| [024](docs/adr/024-progressive-information-to-action-seams.md) | Progressive information-to-action seams | Proposed | Would permit only seller-approved contact, explicit allowlisted external redirects, minimal disclosed handoff payloads, and attributed provider-returned status. ADR-001 remains binding: no WetinDey checkout, funds, payment, order creation, courier dispatch, inventory guarantee, or delivery guarantee. |
| [025](adr/025-trusted-people-remote-presence.md) | Trusted People and relationship-scoped remote presence | Proposed | Would permit only an opaque, non-discoverable relationship after invite/accept, independently expiring reciprocal per-person remote-view grants, and separate profile-display consent. The subject's active ADR-016 foreground lease remains required; selected browsing context is never physical-presence evidence. Block wins bidirectionally, report holds suppress visibility, and ephemeral `Wave` is the first positive/social interaction; safety controls remain mandatory. No follower/popularity trust effect, contact sync, exact location, background tracking, or messaging. The corrected `0012` → contribution `0013+` → operational `0014` gates remain binding. This proposal authorizes no schema, migration, server, UI, provider, pilot, or rollout. |
