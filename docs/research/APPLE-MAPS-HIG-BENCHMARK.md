...
|---|---|---|
| Compact/regular surface behavior is split across a page-local tree and shared shell geometry. | `src/app/page.tsx:1058-1148` owns the sheet header, hit regions, and level-0 scroller; `src/design-system/components/BottomSheet.tsx:1107-1152` owns transformed detents and rounded floating/docked presentation; `src/design-system/components/RegularShell.tsx:91-108` owns the regular island. | **Sheet geometry lane:** `src/design-system/components/BottomSheet.tsx`, `src/design-system/components/RegularShell.tsx`, `src/design-system/components/CompactShell.tsx`, `src/design-system/components/AdaptiveShell.tsx`. `page.tsx` is read-only evidence here because LANES retains it for Maps/Market Details; prove peek/half map context, edge-to-edge large docking, no duplicate surfaces, and AX/pixel order at every detent only after an explicit release. |
...
| The BottomSheet detent handle itself is below the required hit size. | `src/design-system/components/BottomSheet.tsx:1142-1148` renders a button with `pt-2.5 pb-1.5` around a `h-[5px]` bar (about 21px tall) and no `min-h-tap`. | **Sheet hierarchy lane** above owns this exact path; acceptance must include a measured ≥44×44 drag/keyboard target while the visible bar remains slim. |
| Nested rounded surfaces and status pills remain common in offer/list surfaces, and imagery is boxed rather than proven as edge-bleed/blended. | `src/design-system/components/PlaceOfferRow.tsx:69-116` uses a fixed `w-[88px]`, `min-h-[104px]`, `object-cover` frame; `:152-213` nests a rounded article, artwork frame, and status badge; `StatusBadge.tsx:67-69` emits a pill; attribution is separate at `PlaceOfferRow.tsx:210-213`. | **Offer hierarchy and imagery lane:** `src/design-system/components/PlaceOfferRow.tsx`, `src/design-system/components/StatusBadge.tsx`, and only the owning call site. Prove flattened containment, semantic material/elevation, edge bleed/blending, no gutter or sharp-contrast dependency, readable attribution, source-link focusability, and no narrow collision. |
| Avatar semantics distinguish image, initials, and anonymous silhouette in source, but the uploaded image is hidden from AX. | `src/app/page.tsx:1097-1108` renders a 44px Profile target with `Avatar` size 32; `src/app/_components/ProfileSheet.tsx:779-809` falls from image to initials to silhouette, while the visual span at `:789` is `aria-hidden`. | **Identity lane:** `src/app/_components/ProfileSheet.tsx` only. The page callsite and retained Maps self-marker are read-only evidence because `page.tsx` remains under existing Maps/shell ownership. Prove signed-in uploaded avatar, missing/failed-image initials, guest fallback, separate precision/halo, truthful AX names, and no peer publication. |
...
Each proposed implementation path appears in exactly one lane above. Evidence-only gates
and the pathless regular-panel diagnosis deliberately claim no source path; they cannot be
used to bypass the active Presence, Contribution, migration, or retained Maps ownership
locks.
