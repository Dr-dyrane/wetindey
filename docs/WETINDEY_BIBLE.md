...
### Accepted strategic direction: General Search

[ADR-027](docs/adr/027-ai-routed-general-search.md) accepts a post-V1 path from
...
proved faster, clearer, private, accessible, and reliable. Acceptance authorizes no AI
...
[ADR-025](adr/025-trusted-people-remote-presence.md) proposes a separate relationship
capability for a person outside Lagos to view one specifically invited Festac person only
after opaque, non-discoverable invite/QR/handle resolution, mutual invite/accept,
independently expiring reciprocal per-person remote-view grants, relationship-scoped
profile-display consent, and an active ADR-016 foreground lease on the subject. Selected
browsing context is never physical-presence evidence; viewer location is optional, and any
broad viewer disclosure is truthful, separately consented, and never a Festac peer marker.
Block wins bidirectionally, report holds suppress visibility, and ephemeral `Wave` is the
first positive/social interaction; Block, Report, and Close remain mandatory safety/
navigation controls. The proposal forbids public directories, contact sync,
follower/popularity effects, exact coordinates, background tracking, and messaging, and
authorizes no schema, migration, server, UI, provider, pilot, or rollout. The corrected
`0012` → contribution `0013+` → operational `0014` sequencing remains binding. Shared-target
`0014` application is independently proved; runtime and rollout gates remain closed.
...
| Freshness windows are 24h stale / 72h expired | Accepted — see [ADR-006](docs/adr/006-freshness-windows.md) | The numbers remain in force through `FRESHNESS_POLICY`. T1 now sends the same server assessment to cards, detail, Get-It, and narrowed map markers. Migration `0009` is applied and immutable on Preview and Production as part of exact `0000`-`0014`; that database fact does not itself make public confidence admissible or authorize provenance-aware code, promotion, deployment, or runtime enablement. |
...
| Contribution integrity is transactional, moderated, and fail-closed | Accepted — see [ADR-019](docs/adr/019-contribution-integrity-and-moderation.md); implementation unclaimed | Public Food reports enter as immutable pending evidence through one atomic, durably idempotent, server-rate-enforced admission. Unavailable forbids price. Only approved admissible evidence may affect live projections; moderation, projection transitions, decisions, and corrections are append-only and auditable. One report never blanks a seller or result. Implementation is `0013+` after corrected `0012`; acceptance does not apply a migration, reopen reporting, access a database, or authorize deployment |
...
| AI-routed General Search over typed truth capabilities | Accepted — architecture only; implementation separately claimed; see [ADR-027](docs/adr/027-ai-routed-general-search.md) | AI interprets validated intent and constraints; only a live typed WetinDey capability answers from admitted evidence through deterministic rendering. Category fallback, privacy minimization, abstention, and ADR-010's two-complete-vertical abstraction gate remain binding. |
...
- [ADR-025](adr/025-trusted-people-remote-presence.md): proposed relationship-scoped
  remote presence. A person outside Lagos may view one specifically invited Festac person
  only after opaque, non-discoverable invite/QR/handle resolution, mutual invite/accept,
  independently expiring reciprocal per-person remote-view grants, relationship-scoped
  profile-display consent, and an active ADR-016 foreground lease on the subject. Selected
  browsing context is never physical-presence evidence; viewer location is optional, and
  any broad viewer disclosure is truthful, separately consented, and never a Festac peer
  marker. Block wins bidirectionally, report holds suppress visibility, and ephemeral
  `Wave` is the first positive/social interaction; Block, Report, and Close remain
  mandatory safety/navigation controls. No public directory, contact sync, follower/popularity
  effect, exact coordinate, background tracking, or messaging is allowed. This proposal
  authorizes no schema, migration, server, UI, provider, pilot, or rollout; the corrected
  `0012` → contribution `0013+` → operational `0014` sequencing remains binding. The
  shared-target migration is proved; runtime and rollout gates remain closed.
- [ADR-016](docs/adr/016-nearby-user-presence.md): implementation architecture is
  ...
  Founder decision and fresh public-scale safety, legal, and adversarial evidence.
    Production and Preview are independently proved exact through immutable `0014` with
    the same result fingerprint. This records no runtime, pilot, or deployment authorization.
