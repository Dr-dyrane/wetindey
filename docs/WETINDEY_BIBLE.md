...
`0012` → contribution `0013+` → operational `0014` gates remain binding.
...
| Freshness windows are 24h stale / 72h expired | Accepted — see [ADR-006](docs/adr/006-freshness-windows.md) | The numbers remain in force through `FRESHNESS_POLICY`. T1 now sends the same server assessment to cards, detail, Get-It, and narrowed map markers. This does not yet make public confidence admissible: ADR-015 requires observed-only evidence and migration `0009` remains unapplied to shared environments. |
