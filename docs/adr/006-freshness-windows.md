...
**AMENDED 2026-07-23: the 2026-07-17 amendment's citations rotted the same way it warns about.** `src/app/actions.ts` no longer exists (relocated at `0863c78`; the surviving barrel is `src/app/_actions/actions.ts`), so its verification grep now errors on a missing file rather than checking anything; and `getOfferTrustBatch` no longer exists as an exported action (the public wrappers were removed at `8fbd259`; the logic is internal as `getOfferTrustBatchImpl` in `src/lib/food-trust.ts`). Corrected state, verified 2026-07-23: the live `assessTrust` call sites are `src/lib/food-trust.ts` (the read path, consumed in process by `searchItems`, `getPopularItems`, `getPlaceOffers`, and `getOffersNarrowed` in `src/app/_actions/food-actions.ts`) and `src/lib/seo-queries.ts`; the observation WRITE path no longer calls `assessTrust` in the app at all, because admission moved to the contribution runtime and `contribution_admit` derives write-path trust in SQL. The load-bearing hardcoded-constant check is `grep -c 'trustLevel: "high"' src/app/_actions/food-actions.ts` returning 0. The fourth derivation's seed ladder is also history: `src/db/seed.ts` now stamps a flat `trustLevel: "none"`, no ternary ladder. Presentation stays server-owned through `presentOffer` and `OfferPresentation` in the modular item-detail sheet (`offerSignal` survives only as a comment). Everything below stands as the historical record of the 2026-07-17 state.
